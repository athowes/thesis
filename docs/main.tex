...
We have seen that in spatio-temporal statistics, observations have structure.
Each observation should not necessarily be considered as arising from the same distribution as another.
...
Some observations in data are natural to group together.
...
Give equation for a generalised linear mixed model (GLMM) here (may want to introduce GLMs first).
...
Extended latent Gaussian models {[}ELGMs; \textcite{stringer2021fast}{]} facilitate modelling of data with greater non-linearities than an LGM.
...
The model and results for 13 countries are presented in \textcite{howes2023spatio}.
Kathryn has also used the estimates to create a spreadsheet tool (\href{\%60https://hivtools.unaids.org/pse/\%60}{\texttt{https://hivtools.unaids.org/pse/}}) for use in national HIV response planning.
...
On this basis, AGYW have been identified as a priority population for HIV prevention services, and significant investments, such as the DREAMS partnership \autocite{saul2018dreams}, have been made in prevention programming \autocite{global2018measurement}.
...
\caption{Surveys conducted 1999-2018 that were used in the analysis by year, survey type (AIDS Indicator Surveys {[}AIS{]}, Demographic and Health Surveys {[}DHS{]}, the Botswana AIDS Impact Survey 2013 {[}BAIS{]}, and Population-based HIV Impact Assessment {[}PHIA{]} surveys), sample size, and whether the survey included a specific question about transactional sex.}\label{fig:available-surveys}
\end{figure}
...
The sample size for women aged 25-29 is smaller beccause there were 6 DHS surveys which excluded women 25-29 from the transactional sex survey question.

\hypertarget{other-data}{%
\subsection{Other data}\label{other-data}}

As well as the household survey behavioural data, I used estimates of population, PLHIV and new HIV infections stratified by district and age group from HIV estimates published by UNAIDS that were developed using the Naomi model \autocite{eaton2021naomi}.
...
INLA is likely to be the most accurate inference method available because the model, defined over 940 districts, 20 years, 3 age groups, and 3 risk groups, is too large for MCMC to be intractable in reasonable time.
...
The multinomial-Poisson transformation \autocite{baker1994multinomial} reframes a given multinomial logistic regression model as an equivalent Poisson log-linear model.
...
\item
  ...
  To implement the Kronecker product I used the \texttt{group} option in \texttt{R-INLA} \autocite{riebler2011kronecker}.
\end{enumerate}
...
I also tested using the BYM2 model \autocite{simpson2017penalising} in place of the Besag.
...
\item
  ...
  As with the structured spatial random effects, I implemented this Kronecker product using the \texttt{group} option.
\end{enumerate}
