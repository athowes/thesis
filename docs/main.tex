...
4. \textbf{Denominators and demography}
...
\caption{A simple example of Bayesian modelling and computation for the observation model \(y_i \sim \text{Poisson}(\phi)\) for \(i = 1, 2, 3\) with prior \(\phi \sim \text{Gamma}(3, 1)\). Given observed data \(\y = (1, 2, 3)\) the posterior is available in closed form as \(\text{Gamma}(9, 4)\) because the model is so-called conjugate. Though it is unlikely that the best model for a particular problem will have the conjugacy property, conjugate models are still frequently used because of their convenience.}\label{fig:conjugate}
\end{figure}
...
\end{equation}
Unfortunately, it is usually intractable to calculate Equation \eqref{eq:posterior} directly because of the integral \(p(\y) = \int p(\y, \bphi) \text{d}\bphi\) in the denominator, sometimes known as the marginal likelihood or evidence.
...
\tightlist
\item
  \textbf{Sampling algorithms} Known as Monte Carlo \autocite{robert2005monte} algorithms after the casino, these approaches look to generate samples from the posterior distribution.
...
Consider a population of individuals \(j = 1, \ldots, N\) with outcomes of interest \(y_j\).
If a census were run, then the population population mean could be calculated as
\begin{equation}
\mu_N = \frac{1}{N} \sum_{j = 1}^N y_j.
\end{equation}
In practice, only a subset of the individuals will be sampled and their outcomes observed.
Let \(R_j\) be an indicator for whether or not \(y_j\) is recorded.
A weighted estimate of the population mean is
\begin{equation}
\mu_w = \frac{\sum_{j = 1}^N w_j R_j y_j}{\sum_{j = 1}^N w_j R_j},
\end{equation}
where \(w_j > 0\) are survey weights.
The additive error of the weighted estimate may be decomposed as follows \autocite{meng2018statistical}
\begin{equation}
\mu_w - \mu_N = \rho_{Y, R} \times \sqrt{\frac{N - n}{n}} \times \sigma.
\end{equation}
Interpreting these three terms provides useful intuition about survey sampling.
The data quality \(\rho_{Y, R}\) measures the correlation between the outcome of interest and the observation mechanism.
The data quantity \(\sqrt{(N - n)/n}\).
The problem difficultly \(\sigma\) measures the intrinsic difficulty of the estimation problem, and is entirely independent of the sampling or analysis methods used.
...
Nationally representative household surveys, such as the Demographic and Health Surveys (DHS) or Population-based HIV Impact Assessment (PHIA) surveys, provide the highest quality widely relevant information about HIV.
...
This investigation was motivated by a fundamental question encountered during model construction.
Namely, should the model be augmented to capture a feature of the data that we believe exists?
...
Adolescent girls and young women (AGYW) aged 15-29 are a demographic group at increased risk of HIV infection.
...
Over 35 countries have used the Naomi model software (\url{https://naomi.unaids.org}) to produce estimates of HIV indicators \autocite{unaids2023global}.
