...
In particular, the direct estimator given in Equation \eqref{eq:direct}, which does not take into account the sampling probabilities, is biased even in the case of no non-response.
...
A weighted estimate \autocite{hajek1971discussion} of the population mean is then
\begin{equation}
\bar y_\omega = \frac{\sum_{j = 1}^N \omega_j R_j y_j}{\sum_{j = 1}^N \omega_j R_j}. \label{eq:hajek}
\end{equation}

Decomposing the additive error of Equation \eqref{eq:hajek} provides useful intuition as to the utility of survey weighting \autocite{meng2018statistical}
\begin{align}
...
The Kish effective sample size (ESS) \autocite{kish1965survey} is
\begin{equation}
...
\hypertarget{an-adaptive-design-proposal}{%
\subsection{An adaptive design proposal}\label{an-adaptive-design-proposal}}

\textcite{bradley2023alsd}
