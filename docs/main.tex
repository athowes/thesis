...
First, because it allows for principled and flexible integration of data with scientific knowledge.
Second, because it handling of uncertainty is an integral part of the Bayesian paradigm.
...
This is achieved mathematically by the construction of a generative model \(p(\mathbf{y}, \boldsymbol{\mathbf{\phi}})\) for the observed data \(\mathbf{y} = (y_1, \ldots, y_n)\) together with parameters \(\boldsymbol{\mathbf{\phi}} = (\phi_1, \ldots, \phi_d)\), where \(n\) is the dimension of the data and \(d\) is the number of parameters, and the notation \(p(\cdot)\) denotes a probability distribution.
Choice of the particular parameters \(\boldsymbol{\mathbf{\phi}}\) will depend upon the requirements of the analysis.

The model is generative in the sense that one can simulate from it to generate samples \((\mathbf{y}, \boldsymbol{\mathbf{\phi}}) \sim p(\mathbf{y}, \boldsymbol{\mathbf{\phi}})\).
...
Using this procedure, known as a prior predictive check {[}\textcite{gelman2013bayesian}; Chapter 6{]}, models can be built iteratively, with complexity added gradually.
...
Unfortunately, most of the time it is intractable to calculate the posterior distribution directly.
...
The difficulty of performing Bayesian inference in general may be thought of as analogous to the difficulties with calculating integrals.
...
Also known as Monte Carlo methods, sampling algorithms \autocite{robert2005monte} look to generate samples from the posterior distribution
\begin{equation}
\boldsymbol{\mathbf{\phi}}_i \sim p(\boldsymbol{\mathbf{\phi}} \, | \, \mathbf{y}), \quad i \in 1, \ldots M.
\end{equation}
These samples may then be used in any future computations involving the posterior distribution.
...
\begin{equation}
\mathbb{E}(G) = \int G(\boldsymbol{\mathbf{\phi}}) \text{d} \boldsymbol{\mathbf{\phi}} \approx \sum_{i = 1}^M G(\boldsymbol{\mathbf{\phi}}_i).
\end{equation}

Markov chain Monte Carlo (MCMC) methods \autocite{roberts2004general} are perhaps the most popular class of sampling algorithms, and proceed by simulating from an ergodic Markov chain with the posterior distribution as its stationary distribution .
...
HMC uses derivatives of the posterior distribution to generate efficient Metropolis-Hastings \autocite{metropolis1953equation,hastings1970monte} proposal distributions.
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.0698}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Title
...
\bottomrule\noalign{}
\endlastfoot
1 & \href{https://athowes.github.io/thesis/introduction.html}{Introduction} & & \\
