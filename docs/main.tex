...
I began work on this project at the start of my PhD, and only started to make meaningful progress after reading \textcite{stringer2022fast}.
...
To account for\ldots{} the weighting function is also changed such that\ldots{}
...
\tightlist
\item
  Generate a set of nodes \(\bu \in \mathcal{Q}(m, k)\) and weights \(\omega: \bu \to \mathbb{R}\) from a Gauss-Hermite quadrature rule with \(k\) nodes per dimension.
...
Following \textcite{stringer2022fast} I will consider use of the AGHQ rule.
\begin{equation}
...
Does \texttt{R-INLA} use finte differences.
