...
As such, inferences have previously been obtained using an empirical Bayes approximation to full Bayesian inference, where hyperparameters are set using optimisation of the marginal posterior \autocite{casella1985introduction}.
...
Most applications and methodological developments of the INLA method have used \texttt{R-INLA}.
Exceptions include the simplified INLA approach of \textcite{wood2020simplified}, implemented in the \texttt{mgcv} R package, and \textcite{stringer2022fast}.
In pursuit of a suitable inference approach for the Naomi model, I implemented an INLA-like method outside \texttt{R-INLA}, building upon \textcite{stringer2022fast}.
...
\hypertarget{inference-methods-and-software}{%
\section{Inference methods and software}\label{inference-methods-and-software}}
...
\textcite{wood2020simplified} approximates the Laplace marginals without relying so heavily on sparsity via GMRFs and still achieves good performance.
...
\hypertarget{mgcv}{%
\subsubsection{\texorpdfstring{\texttt{mgcv}}{mgcv}}\label{mgcv}}
...
The potential benefits of a more flexible INLA implementation based on AD were noted by \textcite{skaug2009approximate} (a coauthor of TMB) in discussion of \textcite{rue2009approximate}, noting that such a system would be ``fast, flexible, and easy-to-use''.
...
This model is based on that of \textcite{breslow1993approximate}, a modification of \textcite{thall1990some}, and the data are from an epilepsy drug double-blind clinical trial \autocite{leppik1985double}.
...
The linear predictor for this model can be specified in \texttt{R-INLA} by:
...
\hypertarget{aghq-in-moderate-dimensions}{%
\section{AGHQ in moderate dimensions}\label{aghq-in-moderate-dimensions}}
...
The main challenge is that the measurements have to be for marginals, ruling out approaches like PSIS which operate on joint distributions \autocite{yao2018yes}.
