...
Table: (\#tab:agyw-reasons) AYGW are at higher risk of HIV infection as a result of interacting social and biological reasons.
...
(ref:risk-grid) Risk of acquiring HIV depends on both individual-level risk behaviour and population-level HIV incidence.
...
This figure is illustrative, and should not be interpreted quantitatively.
...
Table: (\#tab:risk-groups) HIV risk groups and assumed HIV incidence rate ratio for each risk group relative to AGYW with one cohabiting sexual partner.
...
When the local HIV incidence in the general population is higher, the incidence rate ratio for FSW is lower.

(ref:available-surveys) Surveys used in my analysis by year, survey type, sample size, and whether the survey included a question about transactional sex.
...
I used the multinomial-Poisson transformation to enable inference with `R-INLA`.
...
\end{align}
The transformation works because conditional on their sum Poisson counts are jointly multinomially distributed [@mccullagh1989generalized] as follows
\begin{equation}
...
All random effect precision parameters $\tau \in \{\tau_\beta, \tau_\zeta, \tau_\alpha, \tau_u, \tau_\gamma\}$ were given independent penalised complexity (PC) priors [@simpson2017penalising] with base model $\sigma = 0$ given by $p(\tau) = 0.5 \nu \tau^{-3/2} \exp \left( - \nu \tau^{-1/2} \right)$ where $\nu = - \ln(0.01) / 2.5$ such that  $\mathbb{P}(\sigma > 2.5) = 0.01$.
...
\end{equation}
This working likelihood is implemented by `family = "xPoisson"` in `R-INLA`, which accepts non-integer input.
...
```{r age-disagg-fsw-line, fig.cap="The disaggregation procedure I used produces a plausible age distirbution of FSW by country."}
