...
INLA is likely to be the most accurate inference method available because the model is too large for MCMC to be intractable.
Specifically, the spatio-temporal multinomial logistic regression model is defined over `r n_district` districts, `r n_year` years, `r n_age` age groups, and `r n_group` risk groups.
...
\end{equation}
Using Equations \@ref(eq:multi) and \@ref(eq:mpoisson), the joint distribution of $p(\mathbf{y}_{ita}, m_{ita}) = p(\mathbf{y}_{ita} \, | \, m_{ita})p(m_{ita})$ is then
\begin{align}
