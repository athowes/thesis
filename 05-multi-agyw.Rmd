...
Kathryn has also used the estimates to create a spreadsheet tool ([`https://hivtools.unaids.org/pse/`](`https://hivtools.unaids.org/pse/`)) for use in national HIV response planning.
The tool continues to be updated with inclusion of more countries to the analysis, and extension of the methodology, including to additional risk groups.
...
On this basis, AGYW have been identified as a priority population for HIV prevention services, and significant investments, such as the DREAMS partnership [@saul2018dreams], have been made in prevention programming [@global2018measurement].

The Global AIDS Strategy 2021-2026 [@unaids2021global] was adopted by the United Nations (UN) General Assembly in June 2021.
It proposed stratifying HIV prevention packages to AGYW based on
...
(ref:available-surveys) Surveys conducted 1999-2018 that were used in the analysis by year, survey type (AIDS Indicator Surveys [AIS], Demographic and Health Surveys [DHS], the Botswana AIDS Impact Survey 2013 [BAIS], and Population-based HIV Impact Assessment [PHIA] surveys), sample size, and whether the survey included a specific question about transactional sex.
...
I used household survey data from 13 countries identified by the GFATM as priority countries for implementation of AGYW HIV prevention.
...
Surveys conducted in these countries between 1999 and 2018 in which both women were interviewed about their sexual behaviour and sufficient geographic information was available to locate survey clusters to health districts were included.
...
The HIV incidence rate ratio was assumed to vary by risk group (Table \@ref(tab:risk-groups)).
...
The sample size for women aged 25-29 is smaller beccause there were `r length(giftsvar_exclude_Y025_029)` DHS surveys which excluded women 25-29 from the transactional sex survey question.

### Other data

As well as the household survey behavioural data, I used estimates of population, PLHIV and new HIV infections stratified by district and age group from HIV estimates published by UNAIDS that were developed using the Naomi model [@eaton2021naomi].
...
Exceptions are Cameroon and Kenya, where I conducted analyses one level higher at the department and county levels, respectively.

## Model for risk group proportions

Owing to incomparability across surveys, I took a two-stage modelling approach to estimate the four risk group proportions.
...
To recast this model as an LGM, I used the multinomial-Poisson transformation.
...
INLA is likely to be the most accurate inference method available because the model, defined over `r n_district` districts, `r n_year` years, `r n_age` age groups, and `r n_group` risk groups, is too large for MCMC to be intractable in reasonable time.

#### The multinomial-Poisson transformation

The multinomial-Poisson transformation [@baker1994multinomial] reframes a given multinomial logistic regression model as an equivalent Poisson log-linear model.
...
\end{equation}
<!-- + \delta_{itk} -->
Observation random effects $\theta_{ita} \sim \mathcal{N}(0, 1000^2)$ with a vague prior distribution were included in all models I considered to ensure the multinomial-Poisson transformation was valid.
...
I considered several specifications for the remaining spatial and temporal effects.
...
To implement the Kronecker product I used the `group` option in `R-INLA` [@riebler2011kronecker].

In preliminary testing, I excluded spatial random effects from the model, and found that this negatively effected performance.
I also tested using the BYM2 model [@simpson2017penalising] in place of the Besag.
...
As with the structured spatial random effects, I implemented this Kronecker product using the `group` option.

##### A note on spatio-temporal interaction random effects

I wanted to, and did all four.
This was quite difficult because it required space-time-category interactions.
Building this type of model within formual interface software is challenging.
They ran OK for one country but not for all 13 jointly.
They don't matter much here because there is limited temporal trend.
