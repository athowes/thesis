...
Significant investments, including the DREAMS partnership [@saul2018dreams] and by the Global Fund [@global2018measurement], have been made to support prevention programming.
...
Figure \@ref(fig:risk-grid) illustrates this stylistically.
...
For each survey, respondents were classified into one of four behavioural risk groups $k = 1, 2, 3, 4$ according to reported sexual risk behaviour in the past 12 months (Figure \@ref(fig:category-flowchart)).
...
As with the structured spatial random effects, I implemented this Kronecker product using the `group` option via:
```r
f(year_idx, model = "ar1", group = cat_idx, control.group = list(model = "iid"), ...)
```
Again the more variable with the more complicated model was written first.
...
Table: (\#tab:model-comparison-multinomial) CPO, DIC, and WAIC values for the multinomial logistic regression model specifications with corresponding standard errors.
...
I used the estimates adult (15-49) FSW population size by country from a Bayesian meta-analysis of key population specific data sources [@stevens2023estimating].
...
(ref:model-direct-benefits) The modelled estimates are more plausibly spatially smooth than the direct estimates, and do not have any missing values.
