...
Table: (\#tab:unaids-strategy-prioritisation) Prioritisation strata according to HIV incidence in the general population and behavioural risk.
...
Table: (\#tab:unaids-strategy-targets) Commitments to be met for each intervention, given in terms of the proportion of the prioritisation strata reached. The symbol "-" represents no commitment.
...
if(knitr::is_html_output()) gt_available_surveys
if(knitr::is_latex_output()) gt::as_latex(gt_available_surveys)
```
...
Table: (\#tab:surveys-excluded) All of that surveys that were excluded from the analysis.

## Spatial analysis levels

ana    
D̩partement             Cameroon    
County                   Kenya       
District                 Lesotho     
Distrito                 Mozambique  
Health District + Cities Malawi      
District                 Namibia     
Region                   Eswatini    
District                 Tanzania    
District                 Uganda      
...
Table: (\#tab:survey-questions-ais-dhs) The survey questions included in AIDS Indicator Survey (AIS) and Demographic and Health Surveys (DHS).
...
Table: (\#tab:survey-questions-ais-dhs) The survey questions included in Population-Based HIV Impact Assessment (PHIA) surveys.
