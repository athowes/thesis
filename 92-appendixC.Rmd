...
(ref:tmbstan-epil) Traceplots for the `tmbstan` parameters with the worst effective sample size $\text{ESS}$ and potential scale reduction factor $\hat R$. These were `l_tau_nu` (an $\text{ESS}$ of 377) and `beta[3]` (an $\hat R$ of 1.006).
...
(ref:nuts-loa-loa) Traceplots for the parameters with the worst effective sample size $\text{ESS}$ (Panel A) and potential scale reduction factor $\hat R$.
...
(ref:rhat) The maximum potential scale reduction factor $\hat R$ was `r round(naomi_nuts$rhat_max, 3)`, below the value of 1.05 typically used as a cutoff for acceptable chain mixing. Additionally, the vast majority (`r round(100 * naomi_nuts$rhat_above_threshold, 1)`%) of $\hat R$ values were less than 1.1.

```{r rhat, fig.cap="(ref:rhat)"}
knitr::include_graphics(paste0("resources/naomi-aghq/", resource_version, "/depends/rhat.png"))
```

(ref:ess) The efficiency of the NUTS, as measured by a ratio of effective sample size to iterations, was low for most parameters (Panel A). Many iterations were required for the the effective number of samples (mean `r round(naomi_nuts$ess_mean)`) obtained to be satisfactory (Panel B). 

```{r ess, fig.cap="(ref:ess)"}
knitr::include_graphics(paste0("resources/naomi-aghq/", resource_version, "/depends/ess.png"))
```

(ref:worst-trace) Traceplots for the parameters with the worst effective sample size $\text{ESS}$ (Panel A) and potential scale reduction factor $\hat R$ (Panel B). These were `log_sigma_alpha_xs` (an $\text{ESS}$ of `r round(naomi_nuts$ess_min)`) and `ui_lambda_x[10]` (an $\hat R$ of `r round(naomi_nuts$rhat_max, 3)`). 
...
(ref:rho-a) Pairs plots for `log_sigma_rho_a` and `logit_phi_rho_a`. 

```{r rho-a, fig.cap="(ref:rho-a)"}
knitr::include_graphics(paste0("resources/naomi-aghq/", resource_version, "/depends/rho_a.png"))
```

(ref:alpha-x) Pairs plots for `log_sigma_alpha_x` and `logit_phi_alpha_x`. 
...
(ref:nodes-quantiles-sd) Figure caption.
