...
As discussed I agree that the mismatch between the models favoured by the CPO and IC is surprising and suggestive of a bug.
To investigate this, I began by refitting the four models on a subset of countries for computational convenience.
I found that the observed surprising pattern did not occur, and that both the CPO and IC agreed:
...
To investigate further, I grouped the local observation-specific values by survey and recomputed summary statistics.
I found that in 42 of 46 surveys, the CPO value was higher for Model 4 than Model 3.
...
There were very large differences in the DIC values for the three surveys of the four surveys in Tanzania specifically.
Hence, the strange results are due to differences in how the CPO and DIC/WAIC rank performance of the models in Tanzania.

An additional check that I performed was to look at the values of `cpo$failure` (as discussed on the `R-INLA` website FAQ).
A value of `cpo$failure[i] > 0` for observation `i` indicates the assumptions required for calculation of the CPO are violated, and therefore results may be inaccurate.
I found that, for all models, 30597 observations had `cpo$failure[i] == 0` and 3 observations had `cpo$failure[i] == 1`.
As these three observations cannot have impacted the CPO results significantly (which are consistent across the surveys) I concluded the inaccurate CPO computation is not an issue.
...
All code for the analysis is available at `athowes/thesis` and `athowes/naomi-aghq`.
...
As discussed in the defense, I agree that this is a reasonable concern.
I believe that there are two places in the thesis that I calculate posterior estimates for derived quantities which are a function of multiple elements of the latent field.
These are:
...
2. In Section 6.5.5 where I calculate functions of... exceedance probabilities for the Naomi model.
