...
This is something I highlight at future work in Section 7.2, noting that ideally the method would be included in the `aghq` R package or similar:
...
In the first case, I found (Figure 6.11 and Figure 6.12) that the Laplace marginals approach (which suffers from the concern raised) outperforms the Gaussian marginals approach (which does not) as compared with NUTS (which also takes posterior dependences into account).
