...
As I note in Section 4.5.2, I am concerned that the sample size may be insufficient to draw reasonable conclusions based on the localised simulation results:
...
I would like to prepare this chapter for publication at some point, in which case I would increase the sample size for the simulation study (as I also note in Section 4.3.4.1 regarding the vignette geometries).
One idea I have is to extract features of each area, then look to "model" (or simply look for correlations with) the localised area-specific scores based on these features.
I believe that this could be an interesting approach and help to answer practical questions.
...
This is correct.
The recent work of @chiuchiolo2023joint aims to extend `R-INLA` to use Laplace approximations to estimate subsets of the latent field, beyond merely marginals.
I have added the following note to Section 6.1.3.2:

> An important downside of the Laplace approach is that posterior dependences between posterior marginal draws are not preserved, unlike in the mixture of Gaussians case (Equation (6.15)).
> Recent work using Gaussian copulas (Chiuchiolo, Niekerk, and Rue 2023) aims to retain the accuracy of the Laplace marginals strategy while obtaining a joint approximation.
...
I do not know exactly what went wrong, and think that further study of the use of the Laplace approximation within MCMC routines like HMC could be a fruitful direction (indeed I remember discussing this at the BayesComp conference in 2023).
