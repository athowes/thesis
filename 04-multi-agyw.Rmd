...
The results are described in @howes2023spatio.
Further, Kathryn has created a spreadsheet tool based on the estimates which is beginning to be used by countries to guide policy.
...
The idea is that risk depends on both factors, and therefore prioritisation is more efficient if both are taken into account (Figure \@ref(fig:risk-grid)).
Implementing the strategy is easier when estimates of the size of each population are available.

```{r risk-grid, fig.cap="Risk grid."}
knitr::include_graphics("figures/extra/risk-grid.png")
```
...
First, using all the surveys, I used a spatio-temporal multinomial logistic regression model to model the proportion of AGYW in the risk groups $k \in \{1, 2, 3^{+}\}$.
...
\end{align}
where the number in risk group $k$ is $y_{itak}$, the sample size is $m_{ita} = \sum_{k = 1}^{3^{+}} y_{itak}$, and $k = 1$ is the baseline category.
This model is not an LGM, and it is not possible to fit in `R-INLA`.
...
\end{equation}
In the equivalent model, the sample sizes $m_{ita} = \sum_k y_{itak}$ are treated as random rather than fixed, and take a Poisson distribution
\begin{equation}
...
For simplicity and to avoid numerical issues, by using Besag random effects I decided to fix this proportion to one.
...
To ensure interpretable posterior inferences of random effect contribution, I applied sum-to-zero constraints such that none of the category interaction random effects altered overall category probabilities.
...
\end{equation}
to obtain $y^\star$.
These counts may not be integers, and as such the Poisson likelihood I used in Equation \ref{eq:poisson} is not appropriate.
...
<!-- We considered six logistic regression models (Table \ref{tab:logistic-models}) each including a constant $\beta_0 \sim \mathcal{N}(-2, 1^2)$, country random effects $\zeta_{c} \sim \mathcal{N}(0, \tau_\zeta^{-1})$, and age-country random effects $\alpha_{ac} \sim \mathcal{N}(0, \tau_\alpha^{-1})$. -->
