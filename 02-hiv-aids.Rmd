...
```{r}
pepfar <- 85 * 1000000000
#' From https://www.theglobalfund.org/en/hivaids/
global_fund <- (24.2 + 5) * 1000000000
```

<!-- The extent of HIV as a problem and what is being done -->
The first AIDS cases were reported in Los Angeles in the early 1980s.
Since then, HIV has spread globally, resulting in `r custom_numbers_to_words(deaths)` deaths.
A major international effort has been made to address the epidemic.
Large global health initiatives focused on HIV/AIDS include the Global Fund to Fight AIDS, Tuberculosis, and Malaria (GFATM; \$`r custom_numbers_to_words(global_fund)`) and the President's Emergency Plan for AIDS Relief (PEPFAR; \$`r custom_numbers_to_words(pepfar)`).
...
There were `r custom_numbers_to_words(plhiv2021)` people living with HIV in 2021, the majority of whom taking ART.
...
<!-- Though progress is being made we are not there yet -->
In 2021 there were `r custom_numbers_to_words(deaths2021)` HIV related deaths and `r custom_numbers_to_words(infections2021)` people newly infected with HIV [@unaids2021global].
