...
This section provides required background, before describing the survey design approach used by household surveys in SSA, and the methdos used to analyse this data in this thesis.
...
In the first stage, typically ennumeration areas from a recently conducted census are used as the primary sampling unit, or cluster.
The clusters are assigned strata $h$ by region, as well as by urban-rural status.
After appropriate sample sizes $n_h$ are determined, EAs are sampled with probability proportional to size
\begin{equation}
\pi_{1hj} = n_h \times \frac{N_{hj}}{\sum_j N_{hj}},
\end{equation}
where $N_{hj}$ is the number of households selected in strata $h$ and cluster $j$.
...
Following @chen2014use, the sampling distribution of Equation \@ref(eq:hajek) is arrived at by estimating its variance.
Although this approach acknowledges the complex survey design, it has some important limitations.
Foremost, it ignores clustering structure within the observations $i$.
Furthermore, as a two-step procedure, it fails to fully propagate uncertainty from a Bayesian perspective.
While progress has been made in dealing with survey data, the opening comment of @gelman2007struggles, that "survey weighting is a mess" remains in part relevant.
