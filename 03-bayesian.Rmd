...
It is especially well suited to facing the challenges posed by Section \@ref(surveillance) because it allows for principled and flexible integration of data with domain knowledge.
Additionally, uncertainty is handled as an integral part of the Bayesian paradigm.
...
The model is constructed from two parts, known respectively as the likelihood $p(\mathbf{y} \, | \, \boldsymbol{\mathbf{\phi}})$ and the prior distribution $p(\boldsymbol{\mathbf{\phi}})$.
...
Unfortunately, most of the time it is intractable to calculate analytically.
...
Many computational methods have been developed to tackle the problem of approximating the posterior distribution [@martin2023computing].
...
Modern computational techniques and software have succeeded in abstracting away calculation of the posterior distribution from the analyst for many models.
...
In "The Design of Experiments" @fisher1936design observed likewise that neighbouring crops were more likely to have similar yields.
...
Spatial covariance functions are called isotropic when they apply equally in all directions, and stationary when they are invariant over space.
    ...
    There is a field of time series analysis [@hyndman2018forecasting] to summarise in a couple of sentences.

    Space-time correlation structure [@porcu202130].
    Separable means no space-time correlation can be factorised, nonseparable means it can't.
    Separable correlation could be @knorr2000bayesian I, II, III and IV.
