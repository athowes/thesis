...
The expected log predictive density for new data (ELPD) [@gelman2014understanding] measures the out-of-sample predictive performance and is given by
\begin{equation}
...
\end{align}
Both Equations \@ref(eq:aic-elpd) and \@ref(eq:waic-elpd) are based on within-sample measures, and so overestimate the ELPD.
As such, the information criteria are adjusted downward by a complexity penalty $p_{\texttt{IC}}$.

### Cross-validation

Cross-validation is an alternative way to estimate the ELPD.
Rather than use a complexity penalty, as in Section \@ref(information-criteria), to adjust within-sample estimates of ELPD, cross-validation uses left out data.
...
Another measure of model performance is provided by the evidence $p(\mathbf{y})$, given in Equation \@ref(eq:evidence).
