...
Space and time are important features of data related to HIV.
...
It unites the fields of spatial statistics [@bivand2008applied] and time series analysis [@shumway2017time].

### Properties of spatio-temporal data

Spatio-temporal data have important shared properties.
...
Figure \@ref(fig:st) illustrates both types of observation for space (Panel A\@ref(fig:st)) and time (Panel B\@ref(fig:st)).
...
Section \@ref(st-statistics) showed that in spatio-temporal statistics observations are related to each other, and inappropriate to treat as IID.
This section discuss ways in which the relations between observations can be encoded mathematically.
First, simple structures are discussed, before moving on to more expressive structures.
...
Generalised linear models (GLMs) extend the linear models by allowing the conditional mean $\mu_i$ to be connected to the linear predictor $\eta_i$ via a link function $g$.
\begin{align}
y_i &\sim p(y_i \, | \, \eta_i), \\
...
Similarly, the exponential function $g(\eta) = \exp(\eta)$ is also commonly used to ensure the conditional mean is positive.
GLMs also admit a wider range of likelihoods $p(y_i \, | \, \eta_i)$ than linear models, typically restrictly to the so-called exponential family of distributions.
...
To demonstrate, consider the complete pooling model
\begin{equation}
\eta_i = \beta_0.
\end{equation}
In this model, all observations are assumed to be equivalent.
Alternatively, consider the no pooling model
\begin{equation}
\eta_i = \beta_0 + \beta_1 z_i,
\end{equation}
with $z_i \in \{0, 1\}$ a binary covariate.
Now, there are two groups of observations.
Finally, consider the partial pooling model.
For the partial pooling model, the extent to which information is shared between groups is learnt rather than fixed at the outset, as with the complete or no pooling models.
The parameter $\beta_0$ applies to all groups, and each group is differentiated by a specific value of $u_i$.

Random effects can be structured to share information between some observations more than others.
In spatio-temporal statistics, structured spatial and temporal random effects are often used to encode smoothness.
...
For a review see @vehtari2012survey or @gelman2014understanding.
...
* The log pointwise predictive density (LPPD), the expected log pointwise predictive density for new data (ELPPD) and the expected log predictive density for a new data point (ELPD).

* The Akaike information criterion [AIC; @akaike1973information] is
\begin{align}
  \text{AIC} &= -2 \times \text{elpd}_\text{AIC}, \\
  \text{elpd}_\text{AIC} &=  \log p(\mathbf{y} \, | \, \hat{\boldsymbol{\mathbf{\phi}}}) - p_{\text{AIC}},
\end{align}
where the point estimate $\hat{\boldsymbol{\mathbf{\phi}}}$ is the maximum likelihood estimate.

* The deviance information criterion [DIC; @spiegelhalter2002bayesian] is
\begin{align}
  \text{DIC} &= -2 \times \text{elpd}_\text{DIC}, \\
  \text{elpd}_\text{DIC} &= \log p(\mathbf{y} \, | \, \hat{\boldsymbol{\mathbf{\phi}}}) - p_{\text{DIC}},
\end{align}
where $\hat{\boldsymbol{\mathbf{\phi}}}$ is the posterior mean or mode, and the effective number of parameters is
\begin{equation}
...
\end{equation}
A weakness of the DIC is that it is based on a point estimate [@spiegelhalter2014deviance]
...
\begin{align}
  \text{WAIC} &= -2 \times \text{elpd}_\text{WAIC}, \\
  \text{elpd}_\text{WAIC}& = \sum_{i = 1}^n \log p(y_i \, | \, \mathbf{y}) - p_\text{WAIC}.
\end{align}

### Cross-validation

The above information criteria can be viewed as approximations to cross-validation.
There are three parts of a cross-validation: the way the data is divided, the scoring rule used, the computational method used.

### Scoring rules

Scoring rules [SR; @gneiting2007strictly] measure the quality of probabilistic forecasts.
The log score is one scoring rule.
A proper scoring rule (PSR).
A strictly proper scoring rule (SPSR).
The continuous ranked probability score [CRPS; @matheson1976scoring] generalises the Brier score [@brier1950verification] beyond binary classification.
