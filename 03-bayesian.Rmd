...
During the course of an analysis, many different models can be fit to the same data.
Model comparison methods are used to determine which model is the most suitable to use.
This section focuses on measuring suitability via the model's predictive performance.

Consider new data $\tilde{\mathbf{y}} = (\tilde{y}_1, \ldots, \tilde{y}_n)$ drawn from the true data generating process.
The log predictive density for new data (LPD) [@gelman2014understanding] measures the out-of-sample predictive performance and is given by
\begin{equation}
\text{lpd} = \sum_{i = 1}^n \log p(\tilde y_i \, | \, \mathbf{y}). (\#eq:lpd)
\end{equation}
The expected LPD (ELPD) integrates the LPD over the data generating process to give a measure of expected performance
\begin{equation}
\text{lpd} = \sum_{i = 1}^n \log \int p(\tilde y_i \, | \, \mathbf{y}) p(\tilde y_i) \text{d} \tilde y_i. (\#eq:elpd)
\end{equation}
...
As such, they are adjusted downward by a complexity penalty $p_{\texttt{IC}}$, which varies depending on the particular information criteria.
...
In a leave-out-out (LOO) CV there are $n$ partitions, where each held-out set is a single obsevation.
The LOO-CV estimate of ELPD is
\begin{align}
  \text{elpd}_\texttt{LOO-CV} = \sum_{i = 1}^n \log p(y_i \, | \, \mathbf{y}_i). (\#eq:loo-cv-elpd)
\end{align}
Naively, computing $\text{elpd}_\texttt{LOO-CV}$ requires refitting the model $n$ times.
As such, computational approximations have been developed, most notably importance sampling methods using the the full posterior as a proposal [@vehtari2017practical].

<!-- $K$-fold cross-validation, divide data into $K$ blocks. -->
<!-- Other cross-validiation schemes for specific settings. -->
...
The log score, used above in the ELPD, is one example of a scoring rule, but by no means the only possibility.
In fact, any information criterion (Section \@ref(information-criteria)) or cross-validation strategy (Section \@ref(cross-validation)) can be redefined using a different scoring rule.

One reason the log score (LS) is popular is that it is an example of a strictly proper scoring rule (SPSR).
A scoring rule is called strictly proper when the forecaster gains maximum expected reward by reporting their true probability distribution, and discourages any manipulation.
...
Finally, I mention that the evidence $p(\mathbf{y})$, given in Equation \@ref(eq:evidence), can also be used as a measure of model performance.
...
Bayes factors can be framed as predictive criterion as follows.
