...
One foremost reason is that it allows for principled and flexible integration of prior domain knowledge.
Furthermore, uncertainty over all unknown quantities is handled as an integral part of the Bayesian paradigm.
This section provides a brief and at times opinionated overview.
For a more complete introduction, I recommend @gelman2013bayesian, @mcelreath2020statistical or @gelman2020bayesian.

### Bayesian modelling

The Bayesian approach to data analysis is based on construction of a probability model for the observed data $\mathbf{y} = (y_1, \ldots, y_n)$ joint with parameters $\boldsymbol{\mathbf{\phi}} = (\phi_1, \ldots, \phi_d)$.
Choice of the parameters used in the model depends upon the requirements of the analysis.
...
Models are most naturally constructed from two parts, known respectively as the likelihood $p(\mathbf{y} \, | \, \boldsymbol{\mathbf{\phi}})$ and the prior distribution $p(\boldsymbol{\mathbf{\phi}})$.
...
The extent to which subjective information should be incorporated into the prior distribution, and in doing so influence the posterior distribution, is a central issue.
...
Though subjectivity is most often discussed in the context of the prior distribution specification, we shall see in Section \@ref(hierarchical-lgm-elgm) that the distinction between prior distribution and likelihood is not always clear.
As such, it may be argued that issues of subjectivity are not unique to prior distribution specification, and ultimately the challenge of specifying the data generating process is better thought of more holistically [@gelman2017prior].
...
The evidence $p(\mathbf{y})$ quantifies the probability of obtaining the data under the model.
Although it is easy to evaluate a quantity proportional to the posterior distribution
\begin{equation}
...
Figures \@ref(fig:zmb-maps) and \@ref(fig:zmb-scatter) illustrate the unreliability of direct estimates from small sample sizes, and the way in which a spatial model may be used to overcome this limitation.
...
Describe connection between generalised additive model [GAMs; @wood2017generalized; @hastie1987generalized] and GLMMs.
