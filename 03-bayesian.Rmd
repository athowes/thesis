...
These methods are used throughout this thesis, and are discussed in greater detail in Chapter \@ref(naomi-aghq).
...
The field of spatio-temporal statistics [@cressie2015statistics] is concerned with such observations, which are indexed by spatial and temporal location.
...
Then, Section \@ref(small-area-estimation) describes how these properties facilitate the class of small-area estimation methods that the work in this thesis falls into.
...
Only one observation of a spatio-temporal process can typically be realised.
...
Large data combined with large models make the particular mathematical operation of Bayesian inference challenging for spatio-temporal data.
...
Figures \@ref(fig:zmb-maps) and \@ref(fig:zmb-scatter) illustrate the unreliability of direct estimates from small sample sizes, as well as the way in which a spatial model may be used to partially overcome this limitation.
