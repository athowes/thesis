...
This thesis uses the No-U-Turn sampler [NUTS; @hoffman2014no], a Hamiltonian Monte Carlo [HMC; @duane1987hybrid; @neal2011mcmc] algorithm, as implemented in the `Stan` [@carpenter2017stan] probabilistic programming language (PPL).
...
Monte Carlo methods discussed in Section \@ref(monte-carlo) make use of stochasticity to generate samples from the posterior distribution.
...
These approaches are often faster than Monte Carlo methods, especially for large datasets or models.

One such deterministic approximation is the Laplace approximation.
...
These methods are used often in this thesis.

Variational inference [VI; @blei2017variational] is another noteworthy deterministic approximation.
...
Though popular in some settings, VI lacks theoretical guarantees and is known to often inaccurately estimate posterior variances [@giordano2018covariances].
...
The field of spatio-temporal statistics [@cressie2015statistics] is concerned with such observations indexed by spatial or temporal location.
It unifies the fields of spatial statistics [@bivand2008applied], concerned with observations indexed by space, and time series analysis [@shumway2017time], concerned with observations index by time.

Section \@ref(properties) characterises the shared properties of spatio-temporal data.
Section \@ref(small-area-estimation) describes how these properties facilitate the class of methods called small-area estimation that the work in this thesis falls under.
...
The spatial study region $\mathcal{S} \subseteq \mathbb{R}^2$ used in this thesis is typically a country or collection of countries, and assumed to have two dimensions, corresponding to latitude and longitude.
Observations may be associated to a point $s \in \mathcal{S}$ or area $A \subseteq \mathcal{S}$ in the spatial study region.
Panel A\@ref(fig:st)) illustrates a spatial point and area.
...
Panel B\@ref(fig:st)) illustrates a temporal point and period.
...
This observation was later termed Tobler's first law of geography, that "everything is related to everything else, but near things are more related than distant things" [@tobler1970computer].
As well as space, Tobler's first law applies to time.
Observations made close together in time tend to be similar.

This law can be formalised using space-time covariance functions.
...
SAE methods are more generally useful when data are limited for subpopulations of interest.
...
Section \@ref(correlation-structure) described that in spatio-temporal statistics observations are related to each other, and it is inappropriate to treat data as IID.
This section discusses ways to encode relations between observations mathematically via statistical models.
First, simple structures are discussed such as the linear model, before moving on to more expressive structures.
