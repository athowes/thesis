...
The expected log predictive density for new data (ELPD) is [@gelman2014understanding]
\begin{equation}
\sum_{i = 1}^n \log \int p(\tilde y_i \, | \, \mathbf{y}) p(\tilde y_i) \text{d} \tilde y_i. (\#eq:elpd)
\end{equation}

Information criteria can be constructed to approximate the ELPD.
...
\end{align}
Both Equations \@ref(eq:aic-elpd) and \@ref(eq:waic-elpd) are based on within-sample measures, and so overestimate \@ref(eq:elpd).
Therefore, these information criteria are adjusted downward by varying complexity penalties $p_{\texttt{IC}}$.

### Cross-validation

Cross-validation is an alternative way to estimate the ELPD.
Rather than use a complexity penalty, as in Section \@ref(information-criteria), to adjust within-sample estimates of ELPD, cross-validation operates by leaving out data.
...
The evidence $p(\mathbf{y})$, given in Equation \@ref(eq:evidence), can be used as a measure of model fit.
...
If $B_{01} > 1$ then support is provided for $\mathcal{M_0}$ and if $B_{01} < 1$ then support is provided for $\mathcal{M_1}$.
...
In fact, any information criterion or cross-validation strategy can be adapted to use a different scoring rule.

The log score (LS) is an example of a strictly proper scoring rule (SPSR).
