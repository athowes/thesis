...
(ref:ck-aghq-nuts) A comparison of the posterior means and standard deviations obtained with AGHQ via `aghq` as compared with NUTS via `tmbstan` fitting a CK inferential model to IID synthetic data on the grid geometry (Panel \@ref(fig:geometries)E). For NUTS, the minimum ESS was `r round(aghq_nuts_comparison_diagnostics[6, "min_ess"])`, and the maximum value of the potential scale reduction factor was `r sprintf("%.2f", aghq_nuts_comparison_diagnostics[6, "max_rhat"])`.

```{r ck-aghq-nuts, fig.cap="(ref:ck-aghq-nuts)"}
path <- paste0("resources/beyond-borders/", resource_version, "/depends/ck-aghq-nuts.png")
knitr::include_graphics(path)
```

(ref:fik-aghq-nuts) A comparison of the posterior means and standard deviations obtained with AGHQ via `aghq` as compared with NUTS via `tmbstan` fitting a FIK inferential model to IID synthetic data on the grid geometry (Panel \@ref(fig:geometries)E). For NUTS, the minimum ESS was `r round(aghq_nuts_comparison_diagnostics[5, "min_ess"])`, and the maximum value of the potential scale reduction factor was `r sprintf("%.2f", aghq_nuts_comparison_diagnostics[5, "max_rhat"])`.

```{r fik-aghq-nuts, fig.cap="(ref:fik-aghq-nuts)"}
path <- paste0("resources/beyond-borders/", resource_version, "/depends/fik-aghq-nuts.png")
knitr::include_graphics(path)
```

(ref:ik-aghq-nuts) A comparison of the posterior means and standard deviations obtained with AGHQ via `aghq` as compared with NUTS via `tmbstan` fitting a IK inferential model to IID synthetic data on the grid geometry (Panel \@ref(fig:geometries)E). For NUTS, the minimum ESS was `r round(aghq_nuts_comparison_diagnostics[7, "min_ess"])`, and the maximum value of the potential scale reduction factor was `r sprintf("%.2f", aghq_nuts_comparison_diagnostics[7, "max_rhat"])`.
...
gt_mse <- gt_mse %>%
  gt::tab_caption(caption = "The average mean squared error (MSE) of each inferential model, under different simulation and geometry settings. Entries for FCK and CK on geometry 2 are empty because model was undefined in that case.")
...
gt_crps <- gt_crps %>%
  gt::tab_caption(caption = "The average continuous ranked probability score (CRPS) of each inferential model, under different simulation and geometry settings. Entries for FCK and CK on geometry 2 are empty because model was undefined in that case.")
...
(ref:lengthscale-posteriors) The lengthscale hyperparameter prior and posterior distributions for each of the four considered PHIA surveys (Table \@ref(hiv-surveys)).
...
(ref:map-civ2017phia) Choropleth showing the pointwise CRPS in estimating $\rho_i$ for the Côte d’Ivoire 2017 PHIA survey (Panel \@ref(fig:hiv-surveys)A).
...
(ref:map-mwi2016phia) Choropleth showing the pointwise CRPS in estimating $\rho_i$ for the Malawi 2016 PHIA survey (Panel \@ref(fig:hiv-surveys)B).
...
(ref:map-tza2017phia) Choropleth showing the pointwise CRPS in estimating $\rho_i$ for the Tanzania 2017 PHIA survey (Panel \@ref(fig:hiv-surveys)C).
...
(ref:map-zwe2016phia) Choropleth showing the pointwise CRPS in estimating $\rho_i$ for the Zimbabwe 2016 PHIA survey (Panel \@ref(fig:hiv-surveys)D).
