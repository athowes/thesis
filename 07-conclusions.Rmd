...
To conclude the thesis, this chapter discusses the most important contributions, limitations, promising avenues for future work, and broader reflections about the work.
...
This work is the basis for a tool now being used to prioritise delivery of HIV prevention services by countries in SSA.
The tool is to be updated by UNAIDS as a part of the annual HIV estimates process.
Alongside these applied contributions, Chapter \@ref(multi-agyw) exemplified specification of complex multinomial spatio-temporal models in `R-INLA` using the Poisson-multinomial transformation.
...
This contribution enables practitioners to fit essentially any model using INLA for the first time.
Second, a quadrature rule which naturally extends the applicability of INLA methods to moderate hyperparameter dimension.

## Limitations

The work done in this thesis, particularly Chapters \@ref(beyond-borders) and \@ref(naomi-aghq), focused on generating understanding empirically, using experimentation and computation.
This approach was chosen in part due to the complexity of the models and methods used in this thesis.
...
For example, how should allocation of computational effort be traded-off between additional hyperparameter grid points and more accurate approximations of latent fields?
Theoretical work could be conducted in complement to the above empirical work described above, benefited by exhaustive description of the INLA-like algorithms used.

## Broader reflections

I found conducting the work in this thesis tested the boundaries of available statistical software.
For example, it was challenging, if not impossible, to implement a common model using different software for inference.
As the frequently asked questions section of the `R-INLA` website [@rinla2023faq] notes: "the devil is in the details".
Similarly, I encountered difficulties implementing a desired collection of different models in a common software for inference.
...
The methodological contributions of Chapters \@ref(beyond-borders) and \@ref(naomi-aghq) could eventually lead to marginally more accurate indicator estimates.
However, in the broader context of the HIV response effectiveness these improvements seem of limited consequence.
