...
\end{equation}
where either the estimate of the evidence in Equation \@ref(eq:aghqnormconst) may be reused or a new estimate can be computed.
...
This section is concluded by briefly discussing inclusion or exclusion of the structured additive predictor within the latent field.
...
`TMB` uses the C++ package `CppAD` [@cppaddocumentation] for AD.
Details are provided in Section 3 of @kristensen2016tmb.

Whereas `R-INLA` requires to sparsity structure to be known in advance, `TMB` uses an algorithm to automatically determine the sparsity structure [Section 4.2; @kristensen2016tmb]
...
This section is about implementation of the INLA method using the `TMB` package, with both Gaussian and Laplace marginals, using AD.
The implementation can be described as "universal" in that it is compatible with any model with a `TMB` C++ template, rather than being based on a more restrictive formula interface [@vstrumbelj2023past].
...
I consider inference for the epilepsy GLMM using a range of approaches:
...
Illustrative code is used throughout this section, and will not be used in future sections.
...
This template specifies exactly the same model as `R-INLA` in Section \@ref(epil-inla): in itself a non-trivial task.
The lines with a `DATA` prefix specify the fixed data inputs to be passed to `TMB`.
Examples of data inputs include the data $\mathbf{y}$ and the covariate design matrix.
The lines with a `PARAMETER` prefix specify the parameters $\boldsymbol{\mathbf{\phi}} = (\mathbf{x}, \boldsymbol{\mathbf{\theta}})$ to be estimated.
...
The `TMB` template must first be compiled:
...
Here I use a quasi-Newton method [@dennis1981adaptive] as implemented by `nlminb` from the `stats` R package:
...
The `sdreport` function is used to evaluate the Hessian matrices at a particular value.
Typically, these Hessian matrices are for the hyperparameters, and setting `par.fixed` to the previously obtained `opt$par` returns $\hat{\boldsymbol{\mathbf{H}}}_\texttt{LA}$.
...
The objective function `obj` created in Section \@ref(epil-gauss-eb-tmb) may be directly passed to `aghq` to perform inference by integrating over the hyperparameters using AGHQ.
The argument `k` specifies the number of quadrature nodes to be used per hyperparameters dimension.
...
The Laplace latent field marginal $\tilde p_\texttt{LA}(x_i, \boldsymbol{\mathbf{\theta}}, \mathbf{y})$ may be obtained using `TMB` by setting `random` to $\mathbf{x}_{-i}$ in the `MakeADFun` function call.
However, doing so is not directly possible, as the `random` argument takes a vector of strings as input (e.g. `c("beta", "epsilon", "nu")`) and does not have a native method for indexing.
Instead, the following steps can be taken to modify the `TMB` C++ template to enable the desired indexing:

1. Include `DATA_INTEGER(i)` to pass the index $i$ to `TMB` via the `data` argument.
2. Concatenate the latent field to `PARAMETER_VECTOR(x_minus_i)` and `PARAMETER(x_i)` such that `random` can be set to `x_minus_i`.
3. Include `DATA_IVECTOR(x_lengths)` and `DATA_IVECTOR(x_starts)` to pass the start point and lengths of each subvector of $\mathbf{x}$ via the `data` argument.
The $j$th subvector may then be obtained within the template via `x.segment(x_starts(j), x_lengths(j))`.

As an example of these steps, the modified `TMB` C++ template `epil_modified.cpp` for the epilepsy GLMM is in Appendix \@ref(tmb-modified-epil), and may be compared to the unmodified version.
After suitable alterations are made to `dat` and `param`, it is then possible to obtain the desired objective function via:
...
This section takes an EB approach, fixing the hyperparameters to their modal value
\begin{align}
...
\end{align}
The unnormalised posterior marginal $\tilde p_\texttt{LA}(x_i, \boldsymbol{\mathbf{\theta}}, \mathbf{y})$ (implemented by `obj_i`) may be evaluated at a set of GHQ nodes $z \in \mathcal{Q}(1, l)$ adapted $z \mapsto x_i(z)$ based on the mode and standard deviation of the Gaussian marginal.
Here, $l = 5$ was chosen.
Each evaluation of `obj_i` can be initialised by $\mathbf{x}_{-i}$ set to the mode of the full $N$-dimensional Gaussian approximation $p_\texttt{G}(\mathbf{x} \, | \, \hat{\boldsymbol{\mathbf{\theta}}}_\texttt{LA}, \mathbf{y})$ with the $i$th entry removed $\hat{\mathbf{x}}(\boldsymbol{\mathbf{\theta}})_{-i}$.
...
A normalised posterior can be obtained by computing a de novo posterior normalising constant based on the set of evaluated AGHQ nodes.
...
However, if else statements on updateable data items (as used in `epil_modified.cpp`) are not supported, so this is not yet possible.

#### Laplace marginals and AGHQ with `TMB` {#epil-laplace-aghq-tmb}

The approach taken in Section \@ref(epil-laplace-eb-tmb) may be extended by integrating with respect to the hyperparameters.
To do so, the quadrature nodes used to integrate $p_\texttt{LA}({\boldsymbol{\mathbf{\theta}}}, \mathbf{y})$ can be reused.
\begin{align}
\tilde p(x_i \, | \, \mathbf{y}) &= |\hat{\mathbf{P}}_\texttt{LA}| \sum_{\mathbf{z} \in \mathcal{Q}(m, k)} \tilde p_\texttt{LA}(x_i \, | \, \boldsymbol{\mathbf{\theta}}(\mathbf{z}), \mathbf{y}) \tilde p_\texttt{LA}(\boldsymbol{\mathbf{\theta}}(\mathbf{z}) \, | \, \mathbf{y}) \omega(\mathbf{z}) \\
&\propto \cdots.
\end{align}
Now, each of the $N$ objective functions is evaluated at $k \times l$ points, rather than the $1 \times l$ points required in the EB approach.
...
```r
fit <- rstan::stan(file = "epil.stan", data = dat, chains = 4, refresh = 0)
```
...
Posterior means and standard deviations for the the six regression parameters $\boldsymbol{\mathbf{\beta}}$ from `TMB` were closely aligned to their `R-INLA` analogs (Figure \@ref(fig:beta-mean-sd)).

(ref:beta-mean-sd) The difference between the posterior summary estimate obtained from NUTS (implemented with `tmbstan`) as compared to obtained from a Gaussian or Laplace marginal with quadrature over the hyperparameters (implemented with `R-INLA` or `TMB`).

```{r beta-mean-sd, fig.cap="(ref:beta-mean-sd)"}
knitr::include_graphics("figures/naomi-aghq/beta-mean-sd.png")
```

(ref:intercept-comparison) The ECDF difference for the $\beta_0$ latent field parameter. The difference in error is large depending on use of either the Gaussian or Laplace marginals. Results obtained using `R-INLA` and `TMB` are similar.
...
2. Gaussian marginals and AGHQ with `aghq`
...
This section begins in Section \@ref(naomi-model) by providing a description of the simplified version of Naomi considered in this chapter.
...
(A district-age-sex specific quantity $z_{x,a,s}$ may then be written as $z_i$, and where required the district, age, and sex may be recovered by $x(i) = x$, $a(i) = a$, and $s(i) = s$.)
Let:
...
Their aim to reduce bias by decreasing possible correlation between response and recording mechanism [@meng2018statistical].
...
Equations \@ref(eq:survey-obs), \@ref(eq:weighted-aggregate) and \@ref(eq:kish-ess) are imprecise in the notation used does not reflect the fact that $j$ only runs over individuals within the relevant denominator.
...
This approach is illustrated for the epilepsy example in Section \@ref(epil-gauss-eb-tmb).
...
However, the Naomi model has $m = 24$ hyperparameters.
Although $m = 24$ cannot be described as high-dimensional, it is certainly more than the $m < 4$ or so (low dimensional) parameters which are typical for use of INLA.
...
As short-hand, this method is referred to as EB.
...
After requesting for the generalised binomial distribution used in Equation \@ref(eq:xbin) to be included in `R-INLA` a prototype version was shortly available.
More sustainable to help users to implement their own distributions and models.
