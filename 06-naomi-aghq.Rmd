...
```{r}
resource_version <- "20231205-200840-0d6320ff"
...
Discussion of INLA is concluded by outlining the cases for and against appending the noisy structured additive predictor to the latent field.
...
\end{equation}
As before, $\hat{\mathbf{x}}(\boldsymbol{\mathbf{\theta}}) = \arg\max_{\mathbf{x}} h(\mathbf{x}, \boldsymbol{\mathbf{\theta}})$.

The modified model used by @rue2009approximate extends $\mathbf{x}$ to include a noisy structured additive predictor as follows
\begin{align}
...
@stringer2022fast note that approach has several drawbacks in the case of ELGMs where the matrix $\cdots$ is no longer diagonal, and may be dense.
...
Figures \@ref(fig:epil-time) shows the number of seconds taken to fit the epilespy GLMM model for each approach.
...
(ref:epil-time) The number of seconds taken to perform inference with each method and software implementation.
...
Following @diggle2016model a zero-inflated binomial likelihood was used
\begin{equation}
p(y_i) = (1 - \phi(s_i)) \mathbb{I}(y_i = 0) + \phi(s_i) \text{Bin}(y_i \, | \, n_i, \rho(s_i)) (\#eq:loaloa-likelihood)
\end{equation}
to account for the possibility of structural zeros via a suitability probability $\phi(s_i) \in [0, 1]$, where $s_i \in \mathbb{R}^2$ is the village location and $\rho(s_i) \in [0, 1]$ is the disease prevalence.
...
The zero-inflated likelihood (Equation \@ref(eq:loaloa-likelihood)) is not compatible with `R-INLA`, so only approaches based on the more flexible `TMB` template were included. 
Section 2.2 of @brown2015model demonstrates use of `R-INLA` to fit a simpler LGM model which includes covariates.

@bilodeau2022stochastic implemented this model in `TMB`.
Inference was performed using Gaussian marginals and AGHQ via `aghq` and NUTS via `tmbstan`.
...
| Method         | Software      |
|----------------|---------------|
| Gaussian, AGHQ | `TMB` and `aghq` |
| Laplace, AGHQ  | `TMB` and `aghq` |
| NUTS           | `tmbstan`     |

Table: (\#tab:loaloa-approaches) The inference methods and software considered to fit the Loa loa ELGM.

The `gstat::krige` function [@pebesma2004gstat] was used to implement conditional Gaussian field simulation [@sds; Chapter 12].
Latent field and hyperparameter samples were used in each conditional simulation (Figure \@ref(fig:conditional-simulation-adam-fixed)).
...
(ref:conditional-simulation-diff-fixed) Difference in suitability posterior means $\mathbb{E}[\phi_\texttt{LA}(s)] - \mathbb{E}[\phi_\texttt{G}(s)]$ (Panel A) and prevalence posterior means $\mathbb{E}[\rho_\texttt{LA}(s)] - \mathbb{E}[\rho_\texttt{G}(s)]$(Panel B) between Laplace and Gaussian marginals. As $\beta_\phi$ and $\beta_\rho$ were fixed this difference is as a result in differences in estimation of $u(s)$ and $v(s)$.

```{r conditional-simulation-diff-fixed, fig.cap="(ref:conditional-simulation-diff-fixed)"}
knitr::include_graphics("figures/naomi-aghq/conditional-simulation-diff-fixed.png")
```

(ref:loa-loa-mean-sd) Absolute and relative differences between the Gaussian and Laplace marginal posterior means and standard deviations at each $u(s_i), v(s_i): i \in [190]$.

```{r loa-loa-mean-sd, fig.cap="(ref:loa-loa-mean-sd)"}
knitr::include_graphics("figures/naomi-aghq/loa-loa-mean-sd.png")
```
...
The Naomi model was difficult to efficiently sample from using NUTS.
...
The effective sample size ratios were typically low (Figure \@ref(fig:ess)), and the lowest effective sample size was `r round(naomi_nuts$ess_min, 3)` (2.5% quantile `r round(naomi_nuts$ess_lower, 0)`, 50% quantile `r round(naomi_nuts$ess_median, 0)`, and 97.5% quantile `r round(naomi_nuts$ess_upper, 0)`; Panel \@ref(fig:worst-trace)A).
...
The AR1 log standard deviation $\log(\sigma)$ and logit lag-one autocorrelation parameter $\text{logit}(\phi)$ posteriors were positively correlated (Figure \@ref(fig:rho-a)).
...
On the other hand, BYM2 (Figure \@ref(fig:alpha-x)).
...
(ref:pc-loadings) The principal component loadings based on the eigendecomposition of the inverse curvature.
...
(ref:nodes-samples-comparison) Figure caption.
...
(ref:marginal-sd) Figure caption.
...
The universal INLA implementation can be used to assess the accuracy of INLA for a wider range of models than were previously possible.
