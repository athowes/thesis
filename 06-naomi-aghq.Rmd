...
(ref:hessian-matrix) A submatrix of the full parameter Hessian obtained from `TMB::sdreport` with `getJointPrecision` on the log scale. Entries for the latent field parameters $\boldsymbol{\mathbf{\epsilon}}$ of length 56 and $\boldsymbol{\mathbf{\nu}}$ of length 236 are omitted. Light grey entries correspond to zeros on the real scale.
...
The objective function `obj` created in Section \@ref(epil-gauss-eb-tmb) may be directly passed to `aghq` to perform inference integrating over the hyperparameters using AGHQ.
The argument `k` specifies the number of quadrature nodes to be used per dimension of the hyperparameters.
Here there are two hyperparameters, and `k` is set to three, such that in total there are $3^2 = 9$ quadrature nodes:
...
Draws from the mixture of Gaussians approximating the latent field posterior distribution (Equation \@ref(eq:)) can be obtained by:
...
This is not directly possible, as the `random` argument takes a vector of strings as input (e.g. `c("beta", "epsilon", "nu")`) and has no native method for indexing.
...
As an example, the modified `TMB` C++ template `epil_modified.cpp` for the epilepsy GLMM is in Appendix \@ref(tmb-modified-epil).

It is then possible (after suitable alterations have been made to `dat` and `param`) to obtain the desired objective function via:
...
This section takes an EB approach, such that it remains to evaluate $\tilde p_\texttt{LA}(x_i, \boldsymbol{\mathbf{\theta}}, \mathbf{y})$
