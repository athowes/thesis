...
\end{equation}
The approximate posterior marginal $\tilde p(x_i \, | \, \mathbf{y})$ can be obtained by first normalsiing the marginal Laplace approximation in Equation \@ref(eq:lamarginaljoint)
\begin{equation}
...
Then, integration with respect to the hyperparameters may be performed, similarly to Equation \@ref(eq:mixgaussian)
\begin{align}
...
As such, Equation \@ref(eq:lamarginal) may be represented by its evaluation at a number of nodes, such as those from an AGHQ rule. 
...
First and second derivatives are obtained in `R-INLA` numerically using central finite differences [@fattah2021smart].
...
The `TMB` template may now first be compiled:
...
The objective function `obj` may then be created, implementing $\tilde p_{\texttt{LA}}(\boldsymbol{\mathbf{\theta}}, \mathbf{y})$ and its first and second derivatives:
...
The argument `random` determines which parameters are to be integrated out with a Gaussian approximation.
Here, the parameters `c("beta", "epsilon", "nu")`, or mathematically
\begin{equation}
...
\end{equation}
were selected as the latent field.

The objective function `obj` may then be optimised using any gradient based optimiser, to obtain $\hat{\boldsymbol{\mathbf{\theta}}}_\texttt{LA}$:
...
The `sdreport` function is used to evaluate the Hessian at a particular value.
...
It also is possible to succinctly fit the epilepsy GLMM in a frequentist setting (that is, using improper hyperparameter priors $p(\boldsymbol{\mathbf{\theta}}) \propto 1$) using a formula interface in `glmmTMB`
...
The objective function `obj` created in Section \@ref(epil-gauss-eb-tmb) may be directly passed to `aghq` to perform inference using AGHQ:

```r
start <- c(param$l_tau_epsilon, param$l_tau_nu)
fit <- aghq::marginal_laplace_tmb(obj, k = 3, startingvalue = start)
```

The argument `k` specifies the number of quadrature nodes to be used per dimension, here set to three.
A more complete `aghq` vignette is provided by @stringer2021implementing.

#### Laplace marginals and EB with `TMB` {#epil-laplace-eb-tmb}

The Laplace latent field marginal $\tilde p_\texttt{LA}(x_i, \boldsymbol{\mathbf{\theta}}, \mathbf{y})$ may be obtained by setting `random` to $\mathbf{x}_{-i}$ in the `MakeADFun` function call.
This is not directly possible, as the `random` argument takes a vector of strings as input, and has no native method for indexing.
Instead, the following steps must be taken to modify the `TMB` C++ template:

1. Include `DATA_INTEGER(i)` to pass the index $i$ to `TMB`.
2. Concatenate the latent field to `PARAMETER_VECTOR(x_minus_i)` and `PARAMETER(x_i)`.
3. Include `DATA_IVECTOR(x_lengths)` and `DATA_IVECTOR(x_starts)` to pass the start point and lengths of each subvector of $\mathbf{x}$.
The $j$th subvector may then be obtained using `x.segment(x_starts(j), x_lengths(j))`.

The modified `TMB` C++ template for the epilepsy GLMM is in Appendix \@ref(tmb-modified-epil).

Now it is possible (after suitably altering `dat` and `param`) to obtain the correct objective function via:
...
Posterior means and standard deviations for the the six regression parameters $\boldsymbol{\mathbf{\beta}}$ were near identical using `R-INLA` and `TMB` (Table \@ref(tab:epil-results)).

```{r epil-results}
epil <- readRDS("resources/naomi-aghq/epil.rds")

knitr::kable(epil, caption = "Posterior mean and standard deviation results for the six regression parameters in the epilepsy GLMM.")
```

(ref:epil-time) Time taken.
...
The model structure was as follows.Y
...
This section begins (Section \@ref(naomi-model)) by providing a description of a simplified version of Naomi.
The model is simplified in that it is defined only at the time of the most recent household survey with HIV testing is considered, omitting the nowcasting and temporal projection components of the complete model.
...
Only the most relevant features of the simplified model are included here, with a more exhaustive description left to Appendix \@ref(naomi-math).
...
Let the number of people receiving ART in strata $i$ be $y^{N^\text{ART}}_i$ with corresponding aggregate
\begin{equation}
y^{N^\text{ART}}_I = \sum_{i \in I} y^{N^\text{ART}}_i. (\#eq:art-aggregate)
\end{equation}
...
Let the unobserved count of people in strata $i$ who travel to $x'$ to receive ART be $y^{N^\text{ART}}_{i, x(i) = x, x'}$, such that
\begin{equation}
y^{N^\text{ART}}_i = \sum_{x' \sim x, x' = x} y^{N^\text{ART}}_{i, x(i) = x', x}.
\end{equation}
...
\begin{equation}
y^{N^\text{ART}}_{i, x(i) = x', x} \sim \text{Bin}(N_{i, x(i) = x'}, \pi_{i, x(i) = x', x}).
\end{equation}

Each aggregate attendance observation (Equation \@ref(eq:art-aggregate)) is then modelled using a Gaussian approximation to a sum of binomials.
The sum is over both the strata $i \in I$ and the number of ART clients travelling from district $x(i) = x'$ to $x$ to receive treatment.
The Gaussian approximation is
\begin{equation}
y^{N^\text{ART}}_I \sim \mathcal{N}(\mu^\text{ART}_I, {\sigma^\text{ART}_I}^2),
\end{equation}
where the mean is
\begin{equation}
\mu^\text{ART}_I = \sum_{i \in I} \sum_{x' \sim x, x' = x} N_{i, x(i) = x'} \cdot \pi_{i, x(i) = x', x}, (\#eq:art-aggregate-mean)
\end{equation}
and the variance is
\begin{equation}
{\sigma^\text{ART}_I}^2 = \sum_{i \in I} \sum_{x' \sim x, x' = x} N_{i, x(i) = x'} \cdot \pi_{i, x(i) = x', x} \cdot (1 -  \pi_{i, x(i) = x', x}). (\#eq:art-aggregate-var)
\end{equation}
...
\begin{equation}
\mathbf{y} = (y^{\theta}_I), \quad \theta \in \{\rho, \alpha, \kappa, \rho^\text{ANC}, \alpha^\text{ANC}, N^\text{ART}\}, \quad I \in \mathcal{I}.
\end{equation}
...
Finally, suggestions for future work based on this chapter are presented.
Some suggestions relate more to specific individual contributions of the chapter.
Others take a broader view, or relate to multiple contributions.
...
1. Approximation: Implement the simplified Laplace marginals of @wood2020simplified (Section \@ref(simplified-laplace)).
