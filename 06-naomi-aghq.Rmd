...
This chapter describes a novel Bayesian inference method based on Laplace approximations and quadrature.
Development of the method was motivated the Naomi small-area estimation model.
Over 35 countries have used the Naomi model software ([`https://naomi.unaids.org`](https://naomi.unaids.org)) to produce subnational estimates of HIV indicators [@unaids2023global].
It is challenging to obtain fast and accurate Bayesian inferences in this setting.
For this reason, inferences have previously been obtained using an empirical Bayes strategy.
I began work on this project at the start of my PhD, and only started to make meaningful progress after reading @stringer2022fast.
I am grateful to later have had the opportunity to collaborate with Alex Stringer, including my visiting the University of Waterloo.
...
Here $\boldsymbol{\mathbf{\phi}} = (\phi_1, \ldots, \phi_d)$ are the parameters and $\mathbf{y} = (y_1, \ldots, y_n)$ are the data.
...
\end{equation}
In both Equation \@ref(eq:marginal-posterior-mode) and \@ref(eq:marginal-hessian) dependence on the hyperparameters $\boldsymbol{\mathbf{\theta}}$ is made explicit.
...
distribution.
For the class of latent Gaussian models [@rue2009approximate] the prior on the latent field is Gaussian $\mathbf{x} \sim \mathcal{N}(\mathbf{x} \, | \, \boldsymbol{\mathbf{\theta}})$.
The marginal posterior is then given by...

### Quadrature

Quadrature is an method used to approximate integrals.
As with the Laplace approximation, it is deterministic in that the computational procedure is not intrinsically random, though it might make use of stochastic numerical algorithms.
Let $\mathcal{Q}$ be a set of quadrature nodes $\mathbf{z} \in \mathcal{Q}$ and $\omega: \mathbb{R}^d \to \mathbb{R}$ be a weighting function.
Then, quadrature can be used to estimate the posterior normalising constant by a weighted sum of function evaluations
\begin{equation}
...
The trapezoid rule is a specific example of a quadrature rule, where quadrature nodes are spaced evenly over the domain of interest.
As such, $z_i - z_{i - 1} = \epsilon > 0$ for all $1 < i < k$, and $\omega(z_i) = \epsilon$ for $1 < i < k$ and $\omega(z_i) = \epsilon / 2$ for $i \in \{1, k\}$.
Figure \@ref(fig:trapezoid) shows that in application of the trapezoid rule, the more quadrature nodes are used, the more accurate the estimate of the integrand is.
...
(ref:trapezoid) The trapezoid rule with $k = 5, 25, 125$ quadrature nodes can be used to integrate the function $f(z) = z \sin(z)$ in the domain $[0, \pi]$. Here, the exact solution is $\pi \approx 3.1416$. As $k$ increases and more nodes are used in the computation, the quadrature estimate becomes closer to the exact solution.
...
Gauss-Hermite quadrature [GHQ; @davis1975methods] is a quadrature rule designed to integrate functions of the form $f(\mathbf{z}) = \phi(\mathbf{z}) P_\alpha(\mathbf{z})$ exactly.
Here $\phi(\cdot)$ is a standard multivariate normal density, and $P_\alpha(\cdot)$ is a polynomial of degree $\alpha$.
...
\begin{equation}
\int H_k(z) H_l(z) \phi(z) \text{d} z = \delta_{kl},
\end{equation}
...
In adaptive quadrature, the quadrature nodes and weights depend on the specific integrand under consideration.
Adaptive quadrature rules are particularly important for Bayesian inference problems because the posterior normalising constant $p(\mathbf{y})$ is a function of the data.
No fixed quadrature rule can be expected to effectively integrate every posterior distribution produced by observation of particular data $\mathbf{y}$.
For example, application of the trapezoid rule requires the choice of the start point $a$ and end point $b$, as well as the space between nodes $\epsilon$.
...
When AGHQ is applied to calculation of the posterior normalising constant this transformation is
\begin{equation}
\mathbf{z} \mapsto \hat{\mathbf{P}} \mathbf{z} + \hat{\boldsymbol{\mathbf{\phi}}},
\end{equation}
where $\hat{\mathbf{P}}$ is a matrix decomposition of $\hat{\boldsymbol{\mathbf{H}}} = \hat{\mathbf{P}} \hat{\mathbf{P}}^\top$.
To account for... the weighting function is also changed such that...
...
Two alternatives for the matrix decomposition [@jackel2005note] are the Cholesky and spectral decomposition.
...
Following @stringer2022fast I will consider use of the AGHQ rule.
\begin{equation}
...
Give an example of `R-INLA` use.
Does `R-INLA` use finte differences.

## A universal INLA implementation based on AD

In this section, I implement the INLA method, using the `TMB` package.
This implementation is universal in that it is compatible with any model with a `TMB` C++ template.
This opens the door for application of INLA to models which are not compatible with `R-INLA`.
...
To demonstrate the implementation, consider the epilepsy generalised linear mixed model example of @spiegelhalter1996bugs.
...
<!-- \begin{align} -->
<!-- \hat{\boldsymbol{\mathbf{\theta}}} &= \argmax_{\boldsymbol{\mathbf{\theta}}} {\tilde p_\texttt{LA}(\boldsymbol{\mathbf{\theta}}, \mathbf{y})}, \\ -->
...
<!-- \begin{align} -->
<!-- \hat \mathbf{x}(\boldsymbol{\mathbf{\theta}}) &= \argmax_\mathbf{x} \log p(\mathbf{y}, \mathbf{x}, \boldsymbol{\mathbf{\theta}}), \\ -->
...
<!-- \end{align} -->
<!-- 2. Generate a set of nodes $\bu \in \mathcal{Q}(m, k)$ and weights $\omega: \bu \to \mathbb{R}$ from a Gauss-Hermite quadrature rule with $k$ nodes per dimension. -->
<!-- Adapt these nodes based on the mode and lower Choleksy via $\boldsymbol{\mathbf{\theta}}(\bu) = \hat{\boldsymbol{\mathbf{\theta}}} + \mathbf{L} \bu$. -->
<!-- Use this quadrature rule to calculate the normalising constant $\tilde p_{\texttt{AQ}}(\mathbf{y})$ as follows -->
<!-- \begin{equation} -->
<!-- \tilde p_{\texttt{AQ}}(\mathbf{y}) = \sum_{\bu \in \mathcal{Q}(m, k)} \tilde p_\texttt{LA}(\boldsymbol{\mathbf{\theta}}(\bu), \mathbf{y}) \omega(\bu). \label{eq:evidence1} -->
<!-- \end{equation} -->
<!-- 3. For $i \in [N]$ generate $l$ nodes $x_i(\bv)$ via a Gauss-Hermite quadrature rule $\bv \in \mathcal{Q}(1, l)$ adapted based on the mode $\hat \mathbf{x}(\boldsymbol{\mathbf{\theta}})_i$ and standard deviation $\sqrt{\text{diag}[\hat{\mathbf{H}}(\boldsymbol{\mathbf{\theta}})^{-1}]_i}$ of the Gaussian marginal. -->
<!-- A value of $l \geq 4$ is recommended to enable B-spline interpolation. -->
<!-- For $x_i \in \{ x_i(\bv) \}_{\bv \in \mathcal{Q}(1, l)}$  and $\boldsymbol{\mathbf{\theta}} \in \{ \boldsymbol{\mathbf{\theta}}(\bu) \}_{\bu \in \mathcal{Q}(m, k)}$ calculate the modes and Hessians -->
<!-- \begin{align} -->
<!-- \hat \mathbf{x}_{-i}(x_i, \boldsymbol{\mathbf{\theta}}) &= \argmax_{\mathbf{x}_{-i}} \log p(\mathbf{y}, x_i, \mathbf{x}_{-i}, \boldsymbol{\mathbf{\theta}}), \\ -->
...
<!-- where -->
<!-- $$ -->
<!-- \tilde p_\texttt{LA}(x_i, \mathbf{y}) = \sum_{\bu \in \mathcal{Q}(m, k)} \tilde p_\texttt{LA}(x_i, \boldsymbol{\mathbf{\theta}}(\bu), \mathbf{y}) \omega(\bu). -->
<!-- $$ -->
<!-- and -->
<!-- $$ -->
...
<!-- $$ -->
<!-- \tilde p_{\texttt{AQ}}(\mathbf{y}) = \sum_{\bv \in \mathcal{Q}(1, l)} \tilde p_\texttt{LA}(x_i(\bv), \mathbf{y}) \omega(\bv) -->
<!-- $$ -->

<!-- 5. Given $\{x_i(\bv), \tilde p_\texttt{AQ}(x_i(\bv) \, | \, \mathbf{y})\}_{\bv \in \mathcal{Q}(1, l)}$ create a spline interpolant to each posterior marginal on the log-scale. Samples, and thereby relevant posterior marginal summaries, may be obtained using inverse transform sampling. -->
