...
Figures \@ref(fig:conditional-simulation-phi-diff-fixed) and \@ref(fig:conditional-simulation-rho-diff-fixed) show the difference in posterior means as compared with NUTS depending on use of either Gaussian or Laplace marginals.
...
(ref:loa-loa-time) Time taken placeholder.

```{r loa-loa-time, fig.cap="(ref:loa-loa-time)"}
loa_loa_time_fig <- data.frame(
  "time" = c(1, 180, 1440),
  "method" = c("Gaussian, AGHQ", "Laplace, AGHQ", "NUTS"),
  "software" = c("TMB", "TMB", "tmbstan")
) %>%
  ggplot(aes(x = method, y = time, fill = software)) +
    geom_col() +
    theme_minimal() +
...
ggsave("figures/naomi-aghq/loa-loa-time.png", loa_loa_time_fig, h = 3, w = 6.25)
