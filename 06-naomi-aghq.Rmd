...
This section is about implementation of the INLA method, with both Gaussian and Laplace marginal approximations, using AD via the `TMB` package.
...
Consider inference for the epilepsy GLMM using a range of approaches:

1. INLA with `R-INLA` (Section \@ref(epil-inla)) with varied `strategy` and `int.strategy` controls;
...
Section \@ref(epil-comparison) compares the results from these approaches.
...
The covariates may be obtained and transformed by:
...
This template specifies exactly the same model as `R-INLA` in Section \@ref(epil-inla) (in itself this is non-trivial).
...
The `TMB` user template must first be compiled and linked:
...
Typically, these Hessian matrices are for the hyperparameters.
...
```r
fit <- tmbstan::tmbstan(obj = obj, chains = 4, refresh = 0, laplace = FALSE)
```
