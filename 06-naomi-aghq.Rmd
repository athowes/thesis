...
However, these time points play a limited role in inference as they are not associated to data.
As such, findings about inference for the simplified model are very likely transferable to the complete model.
Only the most relevant features of the simplified model are included here, with a more complete description is left to Appendix \@ref(naomi-math).
After outlining the model, Section \@ref(naomi-elgm) explains why it is not an ELGM [@stringer2022fast] rather than a LGM [@rue2009approximate].
...
It may be described as having three components, corresponding to the three data sources used.
These model components are:
...
\begin{equation}
y^{\theta}_{I} = m^{\theta}_{I} \cdot \hat \theta_{I}, (\#eq:weighted-aggregate)
\end{equation}
where
\begin{equation}
...
HIV data are routinely collected from ANC clinics, to help prevent mother-to-child transmission.
...
\begin{equation}
y^{N^\text{ART}}_I = \sum_{i \in I} y^{N^\text{ART}}_i.
\end{equation}
Let the probability of a person in strata $i$ travelling from district $x(i) = x$ (when $x(i)$ is unspecified, assume $x(i) = x$) to $x'$ to receive ART be
\begin{equation}
\pi_{i, x(i), x'} = \rho_{i, x(i)} \cdot \alpha_{i, x(i)} \times \gamma_{x(i), x'}.
\end{equation}
These probabilities are the product of three probabilities:

1. the probability of having HIV $\rho_{i, x(i)}$,
2. the probability of taking ART $\alpha_{i, x(i)}$,
3. the probability of travelling to district $x'$ to receive ART $\gamma_{x(i), x'}$.

Let the unobserved count of people in strata $i$ who travel to $x'$ to receive ART be $y^{N^\text{ART}}_{i, x(i), x'}$, such that
\begin{equation}
y^{N^\text{ART}}_i = \sum_{x' \sim x, x' = x} y^{N^\text{ART}}_{i, x(i) = x', x}.
\end{equation}
Each of these unobserved counts can be considered as arising from a binomial distribution, with sample size given by the population in strata $i$ (with $x(i) = x'$)
\begin{equation}
y^{N^\text{ART}}_{i, x(i) = x', x} \sim \text{Bin}(N_{i, x(i) = x'}, \pi_{i, x(i) = x', x}) 
\end{equation}

Each aggregate attendance observation is modelled using a Gaussian approximation to a sum of binomials, given by
\begin{equation}
y^{N^\text{ART}}_I \sim \mathcal{N}(\mu^\text{ART}_I, \sigma^\text{ART}_I).
\end{equation}
This sum is over both strata $i \in I$ and the number of ART clients travelling from district $x'(i)$ to $x(i)$.

### Connection to ELGMs {#naomi-elgm}
...
\end{equation}
The structured additive predictor $\boldsymbol{\mathbf{\eta}}$ includes intercept effects, age random effects, and spatial random effects which may be concatenated into the latent field $\mathbf{x}$.
...
Like an LGM, Naomi has a large Gaussian latent field, governed by a smaller number of hyperparameters $m < N$.
However, it has some deviations from the LGM framework, which rather make it an ELGM, and are enumerated below.
Note that when dependence on a specific number of structured additive predictors is given, it is for that item in isolation, rather than when each item is considered together.

1. In the household survey component (Section \@ref(household)) HIV incidence depends on district-level adult HIV prevalence and ART coverage.
This reflects basic HIV epidemiology: HIV incidence is proportional to unsuppressed viral load
\begin{equation}
\lambda \propto \rho (1 - \omega \cdot \alpha),
\end{equation}
with $\omega = 0.7$ a fixed constant.
As a result, each $\log(\lambda_i)$ depends on 28 structured additive predictors where 28 arises by the product of 2 sexes (male and female), 7 age groups ($\{\text{15-19}, \ldots, \text{45-49}\}$), and 2 indicators, HIV prevalence and ART coverage.
Though it is possible to use the `copy` feature in `R-INLA` to include one term in multiple structured additive predictors, this would not be sufficient to allow for this type of model structure.
...
3. In the ANC testing component (Section \@ref(anc)), HIV prevalence and ART coverage depend upon the respective indicators in the household survey component.
Though $\text{logit}(\rho_i)$ and $\text{logit}(\alpha_i)$ are Gaussian, this nonetheless introduces dependence of each mean response on two structured additive predictors.
...
It is possible to specify models with several likelihoods in `R-INLA` by setting `family` to be vector valued.

For the most part, this exercise is academic rather than practical.
Naomi being an ELGM is not the reason that it can't be fit using `R-INLA`, and there is no fundamental reason why the `R-INLA` can not be used to fit ELGMs.
Indeed, some ELGMs may be specified using the advanced features of `R-INLA` [Chapter 6; @gomez2020bayesian], as described above.
Rather, the fundamental reason why Naomi cannot be fit using `R-INLA` is that it is not possible to specify such a complex model using a formula interface.
This fact is not unique to `R-INLA`: any statistical software which does not grant full control over the model's specification will have users requesting additional features to satisfy their use-cases.
This strongly motivates the value of a more univeral INLA implementation (Section \@ref(universal)).
