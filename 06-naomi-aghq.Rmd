...
It is possible to find quadrature rules which are highly accurate when the integrand adheres to certain assumptions [Chapter 4; @press2007numerical].
...
The Bernstein–von Mises theorem states that, under some regularity conditions, as the number of data points increases the posterior distribution convergences to a Gaussian.
...
Multiple methods have been proposed for obtaining $\tilde p(\mathbf{x} \, | \, \mathbf{y})$, varying in both accuracy and computational expense.
...
However, unlike the Gaussian case (Section \@ref(gaussian-marginals)) it is not easy to directly sample from these Laplace approximations.
...
The `inlabru` R package [@bachl2019inlabru] is a notable extension to `R-INLA`, providing a simpler syntax, and capabilities for fitting more general non-linear linear predictor expressions via linearisation and repeat use of `R-INLA`.
