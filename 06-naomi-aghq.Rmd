...
\end{equation}
The variable $x_i$ is excluded from the Gaussian approximation
\begin{equation}
...
\end{equation}
with $(N - 1)$-dimensional posterior mode and $(N - 1) \times (N - 1)$-dimensional Hessian matrix evaluated at the posterior mode
\begin{align}
...
It's possible to reuse the previously obtained posterior normalising constant estimate from Equation \@ref(eq:aghqnormconst).
Alternatively...
...
The quadrature grid used can be chosen by setting `int.strategy` to `"eb"` (empirical Bayes, one quadrature node), `"grid"`, or `"ccd"` [Box-Wilson central composite design; @box1992experimental].
Figure \@ref(fig:inla-grid-demo) demonstrates the two latter integration strategies.

(ref:inla-grid-demo) Consider the function $f(z_1, z_2) = \text{sn}(0.5 z_1, \alpha = 2) \cdot \text{sn}(0.8 z_1 - 0.5 z_2, \alpha = -2)$ as described in Figure \@ref(fig:aghq-demo). Panel A shows the grid method as used in `R-INLA` and detailed in Section 3.1 of @rue2009approximate. Briefly, equally-weighted quadrature points are generated by taking steps of size $\delta_z$ along each eigenvector of the inverse curvature at the mode, until the difference in log-scale function evaluations is below some threshold $\delta_\pi$. Intermediate values are included if they have sufficient log-scale function evaluation. Here, I set $\delta_z = 0.5$ and $\delta_\pi = 2$.
...
Uses state of the art C++ packages such as `Eigen` and `CppAD`.
...
Talk a little about `mgcv::ginla` and what it is capable of.
...
This model is based on that of @breslow1993approximate, a modification of @thall1990some, and the data are from an epilepsy drug double-blind clinical trial [@leppik1985double].
...
The covariates used in the model were age $\texttt{Age}_i$, baseline seizure counts $\texttt{Base}_i$ and an indicator for the final clinic visit $\texttt{V}_4$, which were all centred.
...
The linear predictor for this model can be specified in `R-INLA` by:
...
Second, an approximate Bayesian inference algorithm, which combines AGHQ with PCA to enable use of INLA for moderate dimensional hyperparameters, motivated by a challenging problem in small-area estimation of HIV indicators. 

For the simplified Naomi model in Malawi (Section \@(malawi)) INLA with Gaussian marginals and the PCA-AGHQ quadrature rule was more accurate at inferring posterior distributions of model parameters, across a broad range of metrics, than with an empirical Bayes quadrature strategy, and substantially faster than NUTS.
However, improvements in accuracy for model parameters did not translate into model outputs.
Indeed, we found approximate posterior exceedance probabilities to have systematic inaccuracies, with the potential to meaningfully mislead policy.
...
However, running NUTS for Naomi took days, and in countries with 100s of districts it may simply not be feasible.
...
Section \@ref(pca-aghq) aimed to develop a quadrature rule for moderate dimensions.
It is desirable that this quadrature rule would allocate more computation to dimensions which contribute more to the integral.
...
The first challenge identified was in the use of PCA when the dimensions have different scales. 
Specifically, logit-scale hyperparameters were systematically favoured over those on the log-scale.
...
The main challenge is that the measurements have to be for marginals, ruling out approaches like PSIS which operate on joint distributions [@yao2018yes].
