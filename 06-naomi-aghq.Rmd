...
(ref:loa-loa-data) Empirical prevalence of Loa loa in 190 sampled villages in Cameroon and Nigeria.

```{r loa-loa-data, fig.cap="(ref:loa-loa-data)"}
knitr::include_graphics("figures/naomi-aghq/loa-loa-data.png")
```

@diggle2016model considered a GLMM for the prevalence of the parasitic worm Loa loa.
Observations were collected at $n = 190$ villages in Cameroon and Nigeria (Figure \@ref(fig:loa-loa-data)).
Some areas are thought to be unsuitable for disease transmission.
A zero-inflated binomial likelihood was used
\begin{equation}
y_i \sim
 \begin{cases}
    0, & \text{w.p. } 1 - \phi(s), \\
    \text{Bin}(n_i, \rho(s_i)), & \text{w.p. } \phi(s). \\
  \end{cases}
  (\#eq:loaloa-likelihood)
\end{equation}
...
\begin{align}
  \phi(s) &= \text{logit}^{-1} \left( \beta_\phi + u(s) \right), \\
  \rho(s) &= \text{logit}^{-1} \left( \beta_\rho + v(s) \right),
\end{align}
where $\beta_\phi, \beta_\rho \sim p(\beta)$ are regression coefficients and $u(s), v(s)$ are independent spatial random fields.
...
@bilodeau2022stochastic implemented the zero-inflated model in `TMB` and performed inference using Gaussian marginals and AGHQ via the `aghq` package.
As well, @bilodeau2022stochastic ran NUTS via `tmbstan` but were not able to generate chains which convergened after days of running the sampler.
This section considers inference using three approaches (Table \@ref(tab:loaloa-approaches)): the two approaches considered by @bilodeau2022stochastic and an approach using Laplace marginals.
