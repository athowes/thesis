...
This is undesirable as it could result in underestimating the uncertainty of HIV indicators, ultimately leading to worse policy decisions.

The methods developed in this chapter combine Laplace approximations with adaptive quadrature, and are descended from the integrated nested Laplace approximation (INLA) method of @rue2009approximate.
INLA is a popular method which has enabled fast and accurate Bayesian inferences for a wide range of models, across a large number of scientific fields.
The success of INLA is in large part due to its implementation in the `R-INLA` software.
Use or development of INLA outside the `R-INLA` software has been limited, with some notable exceptions.
...
Potential use cases include applied statisticians who would like to perform inference for complex models, and methods developers looking to extending the INLA methodology.

I began work on this project at the start of my PhD, and only began making meaningful progress after reading @stringer2022fast.
...
The performance of the optimisation algorithm used to obtain the maxima of $h$ (Equation \@ref(eq:posterior-mode)) may be improved by providing access to the gradient of $h$ with respect to $\boldsymbol{\mathbf{\phi}}$.
...
Gauss-Hermite quadrature [GHQ; @davis1975methods] is a quadrature rule designed to integrate functions of the form $f(\mathbf{z}) = \varphi(\mathbf{z}) P_\alpha(\mathbf{z})$ exactly.
...
\end{equation}
corresponds exactly to the Laplace approximation $\tilde p_{\texttt{LA}}(\mathbf{y})$ given in Equation \@ref(eq:la2).
...
Following @stringer2022fast I consider use of AGHQ.
...
Let $\boldsymbol{\mathbf{\theta}}(\mathbf{z}) = \hat{\mathbf{P}}_\texttt{LA} \mathbf{z} + \hat{\boldsymbol{\mathbf{\theta}}}_\texttt{LA}$ where...
\begin{equation}
...
@rue2009approximate use GMRFs.

#### Simplifed INLA

@wood2020simplified don't use GMRFs and still do fine.
...
<!-- Adapt these nodes based on the mode and lower Choleksy via $\boldsymbol{\mathbf{\theta}}(\mathbf{u}) = \hat{\boldsymbol{\mathbf{\theta}}} + \mathbf{L} \mathbf{u}$. -->
<!-- Use this quadrature rule to calculate the normalising constant $\tilde p_{\texttt{AQ}}(\mathbf{y})$ as follows -->
<!-- \begin{equation} -->
<!-- \tilde p_{\texttt{AQ}}(\mathbf{y}) = \sum_{\mathbf{u} \in \mathcal{Q}(m, k)} \tilde p_\texttt{LA}(\boldsymbol{\mathbf{\theta}}(\mathbf{u}), \mathbf{y}) \omega(\mathbf{u}). \label{eq:evidence1} -->
<!-- \end{equation} -->
...
<!-- For $x_i \in \{ x_i(\bv) \}_{\bv \in \mathcal{Q}(1, l)}$  and $\boldsymbol{\mathbf{\theta}} \in \{ \boldsymbol{\mathbf{\theta}}(\mathbf{u}) \}_{\mathbf{u} \in \mathcal{Q}(m, k)}$ calculate the modes and Hessians -->
<!-- \begin{align} -->
