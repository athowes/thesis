...
This chapter describes a novel Bayesian inference method for extended latent Gaussian models, motivated by the Naomi small-area estimation model [@eaton2021naomi].
...
As such, inferences have previously been obtained using an empirical Bayes (EB) approximation to full Bayesian inference.
...
Instead, I propose integrating over the moderate-dimensional hyperparameters using a modified adaptive Gauss-Hermite quadrature [AGHQ; @naylor1982applications] rule, described in Section \@ref(pca-aghq).
...
Section \@ref(universal) builds towards a universally applicable implementation of INLA, where automatic differentiation is used to obtain the derivatives required for the Laplace approximation.
...
Perhaps the most substantial of which is the `inlabru` R package [@bachl2019inlabru], which provides a simplified syntax, and capabilities for fitting more general non-linear linear predictor expressions via linearisation and repeat use of `R-INLA`.
...
The `tmbstan` R package [@monnahan2018no] allows running the Hamiltonian Monte Carlo (HMC) algorithm via Stan, and `aghq` [@stringer2021implementing] which allows use of AGHQ via `mvQuad` [@weiser2016mvquad].
...
The implementation can be described as "universal" in that it is compatible with any model with a `TMB` C++ template, rather than being based on a formula [@vstrumbelj2023past] llike `R-INLA`.
...
Consider a GLMM [@thall1990some; @breslow1993approximate; @spiegelhalter1996bugs] for an epilepsy drug double-blind clinical trial [@leppik1985double] as described below.
...
```r
fit <- aghq::marginal_laplace_tmb(obj, k = 3, startingvalue = c(param$l_tau_epsilon, param$l_tau_nu))
```
...
1. An implementation of INLA using automatic differentiation and compatible with any model which can be expressed as a `TMB` C++ template.
2. An approximate Bayesian inference algorithm, which combines AGHQ with PCA to enable use of INLA for moderate dimensional hyperparameters.
This algorithm was applied to a challenging and important problem in small-area estimation of HIV indicators.

This section discusses these contributions in turn, before moving on to broader suggestions for future work.

### A universal INLA implementation

### PCA-AGHQ with application to Naomi

For the simplified Naomi model in Malawi (Section \@ref(malawi)) INLA with Gaussian marginals and the PCA-AGHQ quadrature rule was more accurate at inferring latent field posterior marginal distributions than with an EB quadrature rule.
Both EB and PCA-AGHQ were substantially faster than NUTS, which took over two days to reach convergence.
However, model output posterior marginal distributions accuracy did not appear to be improved by latent field posterior marginal accuracy.
...
The inaccuracies in model outputs from EB and PCA-AGHQ have the potential to meaningfully mislead policy, and as such where possible gold-standard NUTS results should be computed.
...
NUTS and PCA-AGHQ could be added to the Naomi web interface as an alternative to EB.
...
Though gaining in popularity, the user-base of `TMB` is relatively small, and driven in large part by use of `glmmTMB`.
For users unfamiliar with C++, it can be challenging to use `TMB` directly.

Rather than look disseminate methods via `glmmTMB`, another direction would implement methods into other probabilistic programming languages.
Implementation in Stan would be possible by use of the `bridgestan` package [@bridgestan], which provides access to the methods of a Stan model, together with the adjoint-differentiated Laplace approximation of @margossian2020hamiltonian.
...
Implementation in `NIMBLE` [@de2017programming] is also possible as of version >1.0.0 which includes functionality for automatic differentiation and Laplace approximation [Part V; @nimblemanual] built using `CppAD` like `TMB`, as of  .
Indeed, both the `NIMBLE` and Stan developers are actively looking into implementation of similar algorithms.
...
It is desirable that the quadrature rule allocate more computation to dimensions which contribute more to the integral in question.
...
It would be valuable to include further comparisons in Section \@ref(malawi). 
Two instances stand-out.
First, to compare PCA-AGHQ to other quadrature grids designed for moderate dimensions, such as a CCD.
Second, to conduct inference with an MCMC algorithm better suited to sampling from Naomi than NUTS, such as blocked Gibbs sampling [@geman1984stochastic] or slice sampling [@neal2003slice].
Both of these algorithms are available and customisable, including the choice of block structure, within `NIMBLE`.
...
The universal INLA implementation of Section \@ref(universal) can be used to assess the accuracy of the INLA method for models which have yet to be considered.
