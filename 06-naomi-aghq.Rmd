...
Evidence is synthesised from household surveys and routinely collected health data to generate estimates by district, age, and sex.
...
The posterior distribution encapsulates beliefs about the parameters $\boldsymbol{\mathbf{\phi}} = (\phi_1, \ldots, \phi_d)$ having observed data $\mathbf{y} = (y_1, \ldots, y_n)$, which here I assume to be expressable as vectors.
...
Malawi is a country in sub-Saharan Africa with among the fewest districts.
The Naomi model (Section \@ref(naomi)) was fit to data from Malawi using three inferential approaches:

1. A Gaussian approximation at $\hat{\btheta}_\texttt{LA}$.
2. PCA-AGHQ: a Gaussian approximation mixture at the adapted nodes $\z \in \mathcal{Q}(m, s, k)$.
3. NUTS: the HMC algorithm No-U-Turn Sampling using Stan [@carpenter2017stan] implemented via the `tmbstan` package [@monnahan2018no].
