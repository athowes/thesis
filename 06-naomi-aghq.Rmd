...
The implementation can be described as "universal" in that it is compatible with any model with a `TMB` C++ template, rather than being based on a restrictive formula interface [@vstrumbelj2023past].
...
Section 5 of @osgood2023statistical notes that "`R-INLA` is capable of using higher-quality approximations than `TMB`" and "in return `TMB` is applicable to a wider class of models" yet there is no inherent reason for these capabilities to be in conflict.
...
3. Gaussian marginals and AGHQ with `TMB` (Section \@ref(epil-gauss-aghq-tmb)).
...
(ref:beta-mean-sd) Difference in the posterior summary estimate obtained from NUTS (implemented with `tmbstan`) as compared to obtained from a Gaussian or Laplace marginal with quadrature over the hyperparameters Results from `R-INLA` and `TMB` are highly similar.

```{r beta-mean-sd, fig.cap="(ref:beta-mean-sd)"}
knitr::include_graphics("figures/naomi-aghq/beta-mean-sd.png")
```

(ref:intercept-comparison) The ECDF difference for the $\beta_0$ latent field parameter. An ECDF difference of zero corresponds to obtaining exactly the same results as NUTS, taken to be the gold-standard. The change in ECDF difference is large as to whether Gaussian or Laplace marginals are used. Results obtained for both methods using the `R-INLA` and `TMB` implementations are highly similar.
