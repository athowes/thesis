...
Instead, I propose integrating over the hyperparameters using an modification of adaptive Gauss-Hermite quadrature [@naylor1982applications] for moderate dimensions.
...
Inferences for the latent field can be obtained by approximation of $p(\mathbf{x} \, | \, \mathbf{y})$ using nested application of the quadrature rule [@rue2007approximate]
\begin{align}
...
The posterior marginals for any subset of the complete vector, including $p(x_i \, | \, \mathbf{y})$, can be simply obtained by keeping the relevant entries of $\mathbf{x}$.
...
The quadrature grid used can be chosen by setting `int.strategy` to `"eb"` (empirical Bayes), `"grid"`, or `"ccd"` [Box-Wilson central composite design; @box1992experimental].
Figure demonstrates these three integration strategies.
...
The challenge of connecting this theory to use of the quadrature rule within nested computations remains an open question.
