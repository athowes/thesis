...
Development of the method was motivated the Naomi small-area estimation model [@eaton2021naomi].
Over 35 countries [@unaids2023global] have used the Naomi model software ([`https://naomi.unaids.org`](https://naomi.unaids.org)) to produce subnational estimates of HIV indicators by synthesising evidence from multiple data sources.
The complexity and size of the model makes obtaining fast and accurate Bayesian inferences challenging.
As such, inferences have previously been obtained using an empirical Bayes approximation to full Bayesian inference, where hyperparameters are obtained by optimisation of the marginal posterior [@casella1985introduction]. 
...
Most applications and methodological developments of the INLA method have used `R-INLA`.
Exceptions include the simplified INLA approach of @wood2020simplified, implemented in the `mgcv` R package, and @stringer2022fast.
In pursuit of a suitable inference approach for the Naomi model, I implemented an INLA-like method outside `R-INLA`, building upon @stringer2022fast.
...
The posterior distribution encapsulates beliefs about the parameters $\boldsymbol{\mathbf{\phi}} = (\phi_1, \ldots, \phi_d)$ having observed data $\mathbf{y} = (y_1, \ldots, y_n)$, which I assume to be vectors.
...
This is because the denominator contains a potentially high-dimensional integral, depending on the value of $d \in \mathbb{Z}^+$, over the parameters
\begin{equation}
...
Some approximate Bayesian inference methods, like Markov chain Monte Carlo (MCMC), avoid directly calculating the posterior normalising constant, instead working with the unnormalised posterior distribution
\begin{equation}
...
This integral is known analytically to be $\pi$.
...
For example, using 5, 10, or 20 quadrature nodes per dimension, as in Figure \@ref(fig:trapezoid), in five-dimensions (rather than one) would require 3125, 100000 or 3200000 quadrature nodes respectively.
Though quadrature is embarrassingly parallel, solutions requiring the evaluation of millions quadrature nodes are unlikely to be tractable.
...
This statement requires support using the Bernstein–von Mises theorem in combination with other arguments.
...
\begin{equation}
\tilde p_{\texttt{AQ}}(\mathbf{y}) = | \hat{\mathbf{P}} | \sum_{\mathbf{z} \in \mathcal{Q}(d, k)} p(\mathbf{y}, \boldsymbol{\mathbf{\phi}}(\mathbf{z})) \omega(\mathbf{z}) =  | \hat{\mathbf{P}} | \sum_{\mathbf{z} \in \mathcal{Q}(d, k)} p(\mathbf{y}, \hat{\mathbf{P}} \mathbf{z} + \hat{\boldsymbol{\mathbf{\phi}}}) \omega(\mathbf{z}).
\end{equation}
...
\end{equation}
and corresponds to the Laplace approximation $\tilde p_{\texttt{LA}}(\mathbf{y})$ given in Equation \@ref(eq:la2).
...
The following sections discuss methods for obtaining $\tilde p(\mathbf{x} \, | \, \mathbf{y})$.
...
An alternative more accurate, but more computationally intensive, approach is to calculate a Laplace approximation to the marginal posterior
\begin{equation}
...
The term $\tilde p_\texttt{LA}(x_i, \boldsymbol{\mathbf{\theta}} \, | \, \mathbf{y})$ is obtained by normalising Equation \@ref(eq:lamarginaljoint) using an estimate of the evidence $\tilde p(\mathbf{y})$.
Either the previously obtained posterior normalising constant estimate from Equation \@ref(eq:aghqnormconst) may be reused, or a new estimate may be computed.

#### Simplified Laplace marginals {#simplified-laplace-marginals}

@rue2009approximate use the properties of Gauss-Markov random fields (GMRFs) to efficiently approximate the Laplace marginals.
The simplified approximation is acheived by a Taylor expansion on the numerator and denominator of Equation \@ref(eq:lamarginaljoint) up to third order.
This approach is analogous to correcting the Gaussian approximation in Section \@ref(gaussian-marginals) for location and skewness.
...
A formula interface of the form `y ~ ...` is used to specify the connection between the latent field $\mathbf{x}$ and the structured additive predictor $\boldsymbol{\mathbf{\eta}}$.
...
This section is about implementation of the INLA method, including with Laplace marginals (Section \@ref(laplace-marginals)), using the `TMB` package.
...
The potential benefits of a more flexible INLA implementation based on AD were noted by @skaug2009approximate (a coauthor of TMB) in discussion of @rue2009approximate, noting that such a system would be "fast, flexible, and easy-to-use".
...
It has a significant difference in approximation error depending on use of Gaussian or Laplace marginals.
The second is an example of a model that `R-INLA` is not compatible with.

### Epilepsy example

Consider the epilepsy drug double-blind clinical trial [@leppik1985double] generalised linear mixed model (GLMM) example of @spiegelhalter1996bugs.
...
The linear predictor (Equation \@ref(eq:epil)) is specified in `R-INLA` by:
