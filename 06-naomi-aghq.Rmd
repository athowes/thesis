...
After days of running the sampler, the Markov chain diagnostics were poor.

This section considers inference using three approaches (Table \@ref(tab:loaloa-approaches)).
...
The `gstat::krige` function [@pebesma2004gstat] was used to implement conditional Gaussian field simulation [@sds; Chapter 12] passing in random effect and hyperparameter samples to create each simulation (Figure \@ref(fig:conditional-simulation)).

(ref:conditional-simulation-adam-fixed) Posterior mean of the suitability $\mathbb{E}[\phi(s)]$ (Panel A) and prevalence $\mathbb{E}[\rho(s)]$ (Panel B) random fields.
...
(ref:conditional-simulation-diff-fixed) Figure caption.
...
(ref:loa-loa-mean-sd) Figure caption.
