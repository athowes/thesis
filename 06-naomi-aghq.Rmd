...
The work in this chapter was conducted in pursuit of a fast and accurate Bayesian inference method for the Naomi model [@eaton2021naomi].
This section first provides a description of a simplified version of Naomi, defined only at the time of the most recent household survey with HIV testing is considered (Section \@ref(naomi-model)).
Although the nowcasting and temporal projection components of the complete model are omitted, these time points play a limited role in inference.
As such, conclusions about inference for the simplified model were judged to be very likely transferable to the complete model.
Only the most relevant features of the simplified model are included here, and a more complete description is left to Appendix \@ref(naomi-math).
After outlining the model, reasons are given (Section \@ref(naomi-elgm)) why it is not a LGM [@rue2009approximate], but is an ELGM [@stringer2022fast].
...
\end{equation*}
where $\Psi_i$ are the number of pregnant women, which we assume to be fixed.
...
(Indeed, this section of the model is under active development [@esra2023improved].)
...
Let the number of people recieving ART in strata $i$ be $y^{N^\text{ART}}_i$ with corresponding aggregate $y^{N^\text{ART}}_I = \sum_{i \in I} y^{N^\text{ART}}_i$.
