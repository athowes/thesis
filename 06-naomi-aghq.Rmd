...
The methods in this chapter fall into this latter category.
...
An alternative approach is to approximate the marginal posterior distribution of some subset of the parameters, which do posterior distributions which are well approximated by a Gaussian distribution.
...
Algorithms for fast simulation from a Gaussian distribution include that of @rue2001fast.
...
\end{align}
The approximate posterior marginal $\tilde p(x_i \, | \, \mathbf{y})$ may then be obtained by normalisation of the marginal Laplace approximation, and integration with respect to the hyperparameters, in a similar fashion to Equation \@ref(eq:mixgaussian).

#### Simplified Laplace marginals

@rue2009approximate use the properties of GMRFs to approximate the Laplace marginals.
...
For splines, as with ELGMs, the precision matrix cannot be assumed to be so sparse.
...
The approach used to compute the marginals $\tilde p(x_i \, | \, \mathbf{y})$ can chosen by setting `method` to one of  `"gaussian"` (Section), `"laplace"` (Section) or `"simplified.laplace"` (Section).
The quadrature grid used can be chosen by setting `grid` to one of...
Figure showing a default `R-INLA` grid.
