...
In Naomi, evidence is synthesised from household surveys and routinely collected health data to generate estimates of HIV indicators by district, age, and sex.
...
The methods in this chapter combine Laplace approximations with adaptive quadrature, and are descended from the integrated nested Laplace approximation (INLA) method pioneered by @rue2009approximate.
...
As a result, inferences for the Naomi model have previously been obtained using an empirical Bayes [EB; @casella1985introduction] approximation to full Bayesian inference, with the Laplace approximation implemented by the more flexible Template Model Builder [`TMB`; @kristensen2016tmb] R package.
...
For users of `R-INLA`, the @stringer2022fast approach is analogous to `method = "gaussian"`, while the approach newly implemented in this chapter is analogous to `method = "laplace"`.
...
Recall that inference comprises obtaining the posterior distribution
\begin{equation}
...
\begin{equation}
p_\texttt{G}(\boldsymbol{\mathbf{\phi}} \, | \, \mathbf{y}) = \mathcal{N} \left( \boldsymbol{\mathbf{\phi}} \, | \, \hat{\boldsymbol{\mathbf{\phi}}}, \hat {\mathbf{H}}^{-1} \right) = \frac{| \hat {\mathbf{H}} |^{1/2}}{(2 \pi)^{d/2}} \exp \left( - \frac{1}{2} (\boldsymbol{\mathbf{\phi}} - \hat{\boldsymbol{\mathbf{\phi}}})^\top \hat {\mathbf{H}} (\boldsymbol{\mathbf{\phi}} - \hat{\boldsymbol{\mathbf{\phi}}}) \right).
\end{equation}
...
However, its deviation can be expected to be small if $\log p(\mathbf{y} \, | \, \mathbf{x}, \boldsymbol{\mathbf{\theta}})$ is small [@blangiardo2013spatial].
...
Though quadrature is easily parallelisable, in that function evaluation at each node are entirely independent, solutions requiring the evaluation of millions quadrature nodes are unlikely to be tractable.
...
\hat{\boldsymbol{\mathbf{H}}}_\texttt{LA} &= - \frac{\partial^2}{\partial \boldsymbol{\mathbf{\theta}} \partial \boldsymbol{\mathbf{\theta}}^\top} \log \tilde p_{\texttt{LA}}(\boldsymbol{\mathbf{\theta}}, \mathbf{y}) \rvert_{\boldsymbol{\mathbf{\theta}} = \hat{\boldsymbol{\mathbf{\theta}}}_\texttt{LA}}, (\#eq:hessian2) \\
...
An important downside of the Laplace approach is that posterior dependences between posterior marginal draws are not preserved, unlike in the mixture of Gaussians case (Equation \@ref(eq:mixgaussian)).
Recent work using Gaussian copulas [@chiuchiolo2023joint] aims to retain the accuracy of the Laplace marginals strategy while obtaining a joint approximation.
...
This work was motivated by a setting in which, similar to extended latent Gaussian models [ELGMs; @stringer2022fast], precision matrices are not typically as sparse as GMRFs.
...
Based on these findings, this thesis does not augment the latent field.
...
The approach used to compute the marginals $\tilde p(x_i \, | \, \mathbf{y})$ can be chosen by setting `method` to `"gaussian"` (Section \@ref(gaussian-marginals)), `"laplace"` (Section \@ref(laplace-marginals)) or `simplified.laplace` (Section \@ref(simplified-laplace-marginals)).
...
Perhaps the most substantial is the `inlabru` R package [@bachl2019inlabru].
...
See the `inlabru` package vignettes for additional details.
Further inference procedures which leverage `R-INLA` include INLA within MCMC [@gomez2018markov] and importance sampling with INLA [@berild2022importance].
...
The implementation is universal in that it is compatible with any model with a `TMB` C++ template, rather than based on a restrictive formula interface.
...
|      | Method          |Software|
| -----|-----------------|--------|
| Section \@ref(epil-inla)             | Gaussian latent field marginals, EB over hyperparameters | `R-INLA`      | 
| Section \@ref(epil-inla)             | Gaussian latent field marginals, grid over hyperparameters | `R-INLA`      |
| Section \@ref(epil-inla)             | Laplace latent field marginals, EB over hyperparameters    | `R-INLA`      | 
| Section \@ref(epil-inla)             | Laplace latent field marginals, grid over hyperparameters  | `R-INLA`      |
| Section \@ref(epil-gauss-eb-tmb)     | Gaussian latent field marginals, EB over hyperparameters   | `TMB`         | 
| Section \@ref(epil-gauss-aghq-tmb)   | Gaussian latent field marginals, AGHQ over hyperparameters | `TMB` and `aghq` |
| Section \@ref(epil-laplace-eb-tmb)   | Laplace latent field marginals, EB over hyperparameters    | `TMB`         |
| Section \@ref(epil-laplace-aghq-tmb) | Laplace latent field marginals, AGHQ over hyperparameters  | `TMB` and `aghq` |
| Section \@ref(epil-nuts)             | NUTS           | `tmbstan`     |
| Section \@ref(epil-nuts-rstan)       | NUTS           | `rstan`       |

Table: (\#tab:epil-approaches) The inference methods and software considered to fit the epilepsy GLMM in Section \@ref(epil).
...
The complete algorithm is given in Appendix \@ref(algorithm).
...
(ref:beta-mean-sd) Percentage difference in posterior summary estimate obtained from NUTS as compared to that obtained from a Gaussian (Section \@ref(epil-gauss-aghq-tmb)) or Laplace marginal (Section \@ref(epil-laplace-aghq-tmb)) with AGHQ over the hyperparameters. NUTS results were obtained with `tmbstan`. Results from `R-INLA` and `TMB` are similar, especially for the posterior mean, but do differ in places. Differences could be attributable to bias corrections used in `R-INLA`.
...
| NUTS           | `tmbstan`     | 4 chains of 5000 iterations, with default NUTS settings as implemented in `rstan` [@carpenter2017stan] |

Table: (\#tab:loaloa-approaches) The inference methods and software considered to fit the Loa loa ELGM in Section \@ref(loaloa).
...
After obtaining posterior inferences at each $s_i$, the `gstat::krige` function [@pebesma2004gstat] was used to implement conditional Gaussian field simulation [@sds; Chapter 12] over a fine spatial grid.
Independent latent field and hyperparameter samples were used in each conditional simulation.
For each method (Table \@ref(tab:loaloa-approaches)) 500 conditional Gaussian field simulations were obtained.

#### Results

(ref:conditional-simulation-adam-fixed) Posterior mean of the suitability $\mathbb{E}[\phi_\texttt{LA}(s)]$ (Panel A) and prevalence $\mathbb{E}[\rho_\texttt{LA}(s)]$ (Panel B) random fields computed using Laplace marginals. Inferences over this fine spatial grid were using conditional Gaussian field simulation as implemented by `gstat::krige`.
...
(ref:conditional-simulation-rho-diff-fixed) Difference between the prevalence posterior means with Gaussian marginals $\mathbb{E}[\rho_\texttt{G}(s)]$ and Laplace marginals $\mathbb{E}[\rho_\texttt{LA}(s)]$ to NUTS results. Like the suitability in Figure \@ref(fig:conditional-simulation-phi-diff-fixed), the error the the Gaussian approximation is higher than that of the Laplace approximation. As $\beta_\rho$ was fixed this difference is as a result in differences in estimation of $v(s)$. The diverging colour palette used in this figure is from @thyng2016true.
...
(ref:loa-loa-mean-sd-abs) Absolute difference between the Gaussian and Laplace marginal posterior means and standard deviations to NUTS results at each $u(s_i), v(s_i): i \in [190]$. Relative differences are in Figure \@ref(fig:loa-loa-mean-sd-pct). For close to every node, the Laplace approximation produced a more accurate posterior mean than the Gaussian approximation. For the posterior standard deviation (SD), the picture was more mixed.

```{r loa-loa-mean-sd-abs, fig.cap="(ref:loa-loa-mean-sd-abs)"}
knitr::include_graphics("figures/naomi-aghq/loa-loa-mean-sd-abs.png")
```

(ref:loa-loa-worst-node) The element of the latent field with maximum difference in absolute difference to NUTS for the posterior mean was $u_{184}$. While the Gaussian approximation has substantial error as compared with NUTS, the Laplace approximation is a close match.

```{r loa-loa-worst-node, fig.cap="(ref:loa-loa-worst-node)"}
knitr::include_graphics("figures/naomi-aghq/loa-loa-worst-node.png")
```

For both the suitability and prevalence posterior mean, using Laplace marginals rather than Gaussian marginals substantially reduced error compared to NUTS (Figures \@ref(fig:conditional-simulation-phi-diff-fixed) and \@ref(fig:conditional-simulation-rho-diff-fixed)).
...
At some sites, the differences in ECDF were substantial (Figure \@ref(fig:loa-loa-worst-node)).
...
Figure \@ref(fig:nuts-loa-loa) shows that the results from NUTS were suitable for use, and therefore that this comparison is valid.
...
loa_time <- read_csv("figures/naomi-aghq/loa-loa-time.csv")

loa_time_gaussian <- filter(loa_time, method == "Gaussian, AGHQ")
loa_time_laplace <- filter(loa_time, method == "Laplace, AGHQ")
loa_time_tmbstan <- filter(loa_time, method == "NUTS")

loa_nuts_multiplier <-  signif(100 * loa_time_laplace$time / loa_time_tmbstan$time, 3)
loa_time_tmbstan_hours <- signif(loa_time_tmbstan$time / 60 / 60, 3)
loa_time_laplace_hours <- signif(loa_time_laplace$time / 60 / 60, 3)
loa_time_gaussian <- signif(loa_time_gaussian$time / 60, 3)

knitr::include_graphics("figures/naomi-aghq/loa-loa-time.png")
```

Laplace marginals with AGHQ took `r loa_nuts_multiplier`% of time taken (`r loa_time_tmbstan_hours` hours) by NUTS (Figure \@ref(fig:loa-loa-time)).
That said, Gaussian marginals with AGHQ took less than a minute to run: substantially less than the `r loa_time_laplace_hours` hours taken by the Laplace marginals.
...
Software has been developed for Naomi to allow countries to input their data and interactively generate estimates during week long workshops as a part of a yearly process supported by UNAIDS.
Generation of estimates by country teams, rather than external agencies or researchers, is an important and distinctive feature of the HIV response.
...
To allow interactive review and iteration of model results by workshop participants, any inference procedure for Naomi should ideally be fast and have low memory usage.
Additionally, it should be reliable and automatic, across a range of country settings.
Naomi is a complex model, comprised of multiple linked generalized linear mixed models (GLMMs), and as such these requirements present a challenging Bayesian inference problem.

This section begins (Section \@ref(naomi-model)) by describing a simplified version of Naomi.
The model is simplified in that it is defined only at the time of the most recent household survey with HIV testing.
...
Naomi synthesises data from three sources to estimate HIV indicators at a district-level, by age and sex.
...
(Indeed, this section of the model remains a challenge, and is under active development [@esra2024improved].)
...
\begin{equation}
\pi_{i, x(i) = x, x'} = \rho_{i} \alpha_{i} \gamma_{x(i) = x, x'}. (\#eq:receive-art)
\end{equation}
...
As a result, a quadrature rule which does not scale exponentially is required for integrating out the Naomi model hyperparameters.
...
\begin{equation}
\omega(\mathbf{z}) = \prod_{j = 1}^s \omega_s(z_j) \times \prod_{j = s + 1}^m \omega_1(z_j).
\end{equation}
...
2. Gaussian marginals and PCA-AGHQ with `TMB`.
...
<!-- X. Laplace marginals and EB with `TMB`. -->
...
For the PCA-AGHQ quadrature grid, a Scree plot based on the spectral decomposition of $\hat {\mathbf{H}}_\texttt{LA}^{-1}$ (as defined in Equation \@ref(eq:hessian2)) was used to select the number of principal components to keep (Figure \@ref(fig:total-variation)).
...
The principal component (PC) loadings (Figure \@ref(fig:pc-loadings)) provide interpretable information about which directions had the greatest variation.
Many of the first PC loadings are sums of two hyperparameters.
As such, there is some redundancy in the hyperparameter parameterisation, supporting the findings of Section \@ref(naomi-nuts) regarding correlation structure in the hyperparameter posterior.
...
This discrepancy occurred due to logistic hyperparameters naturally having higher posterior marginal standard deviation than logarithmic hyperparameters (Figure \@ref(fig:marginal-sd)).

### Time taken

(ref:naomi-time) The number of hours taken to perform inference for the Naomi ELGM (Section \@ref(naomi-model)) using each approach.
...
Inference with NUTS took `r naomi_time_tmbstan_hours` hours, while inference with GPCA-AGHQ took `r naomi_time_aghq_hours` hours and GEB just `r naomi_time_tmb_minutes` minutes (Figure \@ref(fig:naomi-time)).
Both the NUTS and GPCA-AGHQ algorithms can be run under a range of settings, trading off accuracy and runtime.

### Inference comparison

Posterior inferences from GEB, GPCA-AGHQ and NUTS were compared using point estimates (Section \@ref(point-estimates)) and distributional quantities (Section \@ref(distributional-quantities)).
...
```{r}
mmd <- readRDS(paste0("resources/naomi-aghq/", resource_version, "/depends/mmd.rds"))
...
Those of GPCA-AGHQ (`r signif(mmd$mmd_aghq@mmdstats[1], 2)` and `r signif(mmd$mmd_aghq@mmdstats[2], 2)`) were just `r first_order_pct`% and `r third_order_pct`% lower.
...
Both GPCA-AGHQ and GEB had substantial error as compared to results from NUTS, however, particularly for girls and women.
This discrepancy in accuracy by sex may be caused by interactions between the household survey and ANC components of the model creating a more challenging posterior geometry.

#### Finding strata with high incidence {#one-inc}
...
Again, both methods had significant error.
Unlike in Section \@ref(second90), there was little difference in error by sex.
...
Section \@ref(universal) made important progress towards adding INLA to the integration tools easily accessible using `TMB`.
Reaching this milestone would be of significant value to both applied and methodological researchers.

The implementation is not intended to replace `R-INLA`, and indeed for the majority of users a formula-based interface is preferred.
Both formula-based and universal statistical tools have value, as they inhabit different use-cases.
For the NUTS algorithm, a universal interface is available via Stan, and packages such as `brms` [@brms] and `rstanarm` [@rstanarm] enable researchers to fit common models using a formula interface.
...
For the simplified Naomi model applied to data from Malawi, GPCA-AGHQ more accurately inferred latent field posterior marginal distributions than GEB.
However, model output posterior marginals did not see the same improvements.
Approximate posterior exceedance probabilities from both GEB and GPCA-AGHQ had systematic inaccuracies as compared with NUTS.
...
To empower users, GPCA-AGHQ and NUTS could be added to the Naomi web interface ([`https://naomi.unaids.org`](https://naomi.unaids.org)) as alternatives to GEB.
...
4. Finally, it would be of substantial interest to implement the Naomi model using the iterative INLA method via `inlabru`.
...
4. Finally, it may be possible to achieve better performance by pruning and prerotation, as discussed by @jackel2005note.
...
(ref:package-downloads) Monthly R package downloads from the Comprehensive R Archive Network (CRAN) for `brms`, `glmmTMB`, `nimble`, `rstan` and `TMB`, obtained using the `cranlogs` [@cranlogs] R package. Unfortunately, `R-INLA` is not available from CRAN, and so could not be included in this figure. The official `rstan` documentation recommends installation of a development version hosted outside CRAN. As such, this metric may underestimate the popularity of `rstan`.
