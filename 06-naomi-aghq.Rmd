...
As such, inferences have previously been obtained using an empirical Bayes strategy, where hyperparameter uncertainty is not taken into account fully.
I began work on this project at the start of my PhD, and only started to make meaningful progress after reading @stringer2021fast.
...
\end{equation}
or some way to compute relevant functions of it, where $\boldsymbol{\mathbf{\phi}} = (\phi_1, \ldots, \phi_d)$ are the parameters and $\mathbf{y} = (y_1, \ldots, y_n)$ are the data.
...
Let $\boldsymbol{\mathbf{\phi}} = (\mathbf{x}, \boldsymbol{\mathbf{\theta}})$ and consider a three-stage three-stage hierarchical model of the form
\begin{align}
...
\begin{equation}
\hat{\mathbf{x}}(\boldsymbol{\mathbf{\theta}}) = \arg\max_{\mathbf{x}} h(\mathbf{x}, \boldsymbol{\mathbf{\theta}}) (\#eq:posterior-mode)
\end{equation}
and Hessian matrix evaluated at the posterior mode
\begin{equation}
\hat {\mathbf{H}}(\boldsymbol{\mathbf{\theta}}) = - \frac{\partial^2}{\partial \mathbf{x} \partial \mathbf{x}^\top} h(\mathbf{x}, \boldsymbol{\mathbf{\theta}}) \rvert_{\mathbf{x} = \hat{\mathbf{x}}(\boldsymbol{\mathbf{\theta}})}. (\#eq:hessian)
\end{equation}
In both Equation \@ref(eq:posterior-mode) and \@ref(eq:hessian) dependence on the hyperparameters $\boldsymbol{\mathbf{\theta}}$ is made explicit.
...
Let $\mathcal{Q}$ be a set of quadrature nodes $z \in \mathcal{Q}$ and $\omega: \mathbb{R} \to \mathbb{R}$ be a weighting function.
Quadrature can be used to estimate the posterior normalising constant as follows
\begin{equation}
\tilde p_{\mathcal{Q}}(\mathbf{y}) = \sum_{\mathbf{z} \in \mathcal{Q}} p(\mathbf{y}, \mathbf{z}) \omega(\mathbf{z}).
\end{equation}

To illustrate how quadrature works, consider integrating the univariate function $f(z)$ between $z = a$ and $z = b$.
A quadrature approximation of this integral is given by
\begin{equation}
\int_{a}^b f(z) \text{d} z \approx \sum_{z \in \mathcal{Q}} f(z) \omega(z),
\end{equation}
where $\mathcal{Q} = \{z_1, \ldots z_k\}$ is a set of quadrature nodes and $\omega: \mathbb{R} \to \mathbb{R}$.
The trapezoid rule is a simple examples of a quadrature rule, where quadrature nodes are spaced evenly across the domain of interest such that $z_i - z_{i - 1} = \epsilon > 0$ for all $1 < i < k$, and $\omega(z_i) = \epsilon$ for $1 < i < k$ and $\omega(z_i) = \epsilon / 2$ for $i \in \{1, k\}$.
Figure \@ref(fig:trapezoid) shows that the more quadrature nodes are used, the more accurate the estimate of the integrand is.
Under some regularity conditions on $f$, then as $\epsilon \to 0$ the quadrature estimate using the trapezoid rule converges to the true value of the integral.
...
The number of quadrature nodes required grows exponentially with the dimension.
...
First, to construct the univariate GHQ for $z \in \mathbb{R}$, let $H_k(z)$ be the $k$th Hermite polynomial
\begin{equation}
