...
Derivatives are calculated in `R-INLA` numerically using central finite differences.
...
The objective function `obj`, implementing $\tilde p_{\texttt{LA}}(\boldsymbol{\mathbf{\theta}}, \mathbf{y})$ and its first and second derivative, may then be created:
...
The objective function `obj` may then be optimised using any gradient based optimiser in R, to obtain $\hat{\boldsymbol{\mathbf{\theta}}}_\texttt{LA}$:
...
The `sdreport` function is used to evaluate the Hessian at a particular value given by `par.fixed`, which here I use to obtain $\hat{\boldsymbol{\mathbf{H}}}_\texttt{LA}$:
...
For interest in the efficiency of `tmbstan`, the epilepsy model was also implemented in `Stan`.
...
`tmbstan` and `rstan` were comparable.
