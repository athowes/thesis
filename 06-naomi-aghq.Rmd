...
Case counts $y_i$ were obtained from field studies in $n = 190$ villages in Cameroon and Nigeria [@schluter2016using; Figure \@ref(fig:loa-loa-data)].
Some areas are thought to be unsuitable for disease transmission, and as a result there are a high number of villages with zero prevalence. 
...
\end{equation}
to account for the possibility of structural zeros via a suitability probability $\phi(s) \in [0, 1]$.
The prevalence and suitability log-odds were modelled according to
\begin{align}
  \text{logit}[\phi(s)] &= \beta_\phi + u(s), \\
  \text{logit}[\rho(s)] &= \beta_\rho + v(s).
\end{align}
The regression coefficients $\beta_\phi, \beta_\rho$ were given $\mathcal{N}(0, 1000)$ prior distributions.
Independent Gaussian processes $u(s), v(s)$ were specified with a shared Matern kernel.
The hyperparameters $\boldsymbol{\mathbf{\theta}} = (\sigma, \rho)$ of the kernel were given penalised complexity [@simpson2017penalising; @fuglstad2019constructing] prior distributions with
\begin{align}
\mathbb{P}(\sigma < 4) &= 0.975, \\
\mathbb{P}(\rho < 200\text{km}) &= 0.975.
\end{align}

The model was implemented by @bilodeau2022stochastic in `TMB`.
Inference was performed using Gaussian marginals and AGHQ via `aghq` and NUTS via `tmbstan`.
The Markov chain diagnostics were poor, even after days of running the sampler.

In this section inference is considered using three approaches (Table \@ref(tab:loaloa-approaches)): the two approaches considered by @bilodeau2022stochastic and an approach using Laplace marginals.
It is not possible to fit this model in `R-INLA` because of the zero-inflated likelihood (Equation \@ref(eq:loaloa-likelihood)).
Section 2.2 of @brown2015model demonstrates use of `R-INLA` to fit a simpler model without zero-inflation, which includes covariates.
...
Following @bilodeau2022stochastic, the `geostatsp::RFsimulate` [@brown2015model] was used as a wrapper to the `RandomFields` R package [@schlather2015analysis] to implement conditional Gaussian field simulation with hyperparameter samples.
