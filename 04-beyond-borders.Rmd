...
The investigation was motivated by a commonly encountered question: should the model be altered to capture a reasonable, albeit hypothetical, feature of the data?
...
For mapping of other infectious diseases, such as Malaria where transmission is driven by more predictive and easily-measurable environmental factors, explanatory covariates are more easily available and modelling spatial variation is less pertinent [@weiss2015re; @bhatt2015effect].
...
As a result, it has been widely used, including:
...
* to map mortality from cancers [@rashid2023inequalities], injuries [@parks2020anomalously] and air pollution [@bennett2019particulate];
...
From most to least spatially regular, these were:

* a $6 \times 6$ lattice grid;
* the 33 districts of Côte d'Ivoire;
* the 36 congressional districts of Texas.

For each of the three spatial random effect models and seven geometries 100 simulations were generated, resulting in 2100 total synthetic data.
...
| CK    | $\mathbf{u} \sim \mathcal{N}(0, \tau_u^{-1} \mathbf{K})$ with $K_{ij} = k(c_i, c_j)$ as in Section \@ref(centroid-kernel) with length-scale prior distribution $l \sim \mathrm{Inv{\text-}Gamma}(a, b)$ |
| FIK   | $\mathbf{u} \sim \mathcal{N}(0, \tau_u^{-1} \mathbf{K})$ with $K_{ij} = \cdots$ as in Section \@ref(integrated-kernel) with fixed length-scale $l$ |
| IK    | $\mathbf{u} \sim \mathcal{N}(0, \tau_u^{-1} \mathbf{K})$ with $K_{ij} = \cdots$ as in Section \@ref(integrated-kernel) with length-scale prior distribution $l \sim \mathrm{Inv{\text-}Gamma}(a, b)$ |
...
w_{ij} = \frac{1}{\pi_{ij}}
\end{equation}
were used to account for the complex survey design whereby an individual $j$ in area $i$ has probability $\pi_{ij}$ of being included in the survey as follows.
The Kish effective sample size [@kish1965survey]
\begin{equation}
m_i^\star = \frac{(\sum_j w_{ij})^2}{\sum_j w_{ij}^2}.
\end{equation}
The effective number of cases $y_i^\star$ may be thought of as what would have been observed had the survey been a simple random sample.
