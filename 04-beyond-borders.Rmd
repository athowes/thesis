...
In this case, the feature in question pertains to the spatial correlation structure between areas.
...
* $\tau_u$ is the marginal precision of $\mathbf{u}$;
* $\phi \in [0, 1]$ gives the proportion of the marginal variance explained by each component;
* $\mathbf{w}^\star$ is a scaled version of $\mathbf{w}$ with precision matrix given by the scaled structure matrix $\mathbf{R}^\star$.
...
#### Possible mismatch to data generating process
...
Models were assessed using two methods:

* the continuous ranked probability score [CRPS; @matheson1976scoring];
* their calibration.

The CRPS is a strictly proper scoring rule [SPSR; @gneiting2007strictly] in that it is uniquely maximised by truthful representation of beliefs about the parameter in question.
Any non-proper scoring rule rewards misrepresenting beliefs to maximise the score.
Let the parameter $\phi$ have posterior marginal $f(\phi) = p(\phi \, | \, \mathbf{y})$ with cumulative distribution function $F$.
If the true value of $\phi$ used in the simulation was $\omega$ then the CRPS is
\begin{equation}
\text{CRPS}(f, \omega) = \int_{-\infty}^{\infty} (F(\phi) - \mathbb{I} \{\phi \geq \omega \} )^2 \text{d}\phi. (\#eq:crps)
\end{equation}
Let $\phi_s$ be samples $s \in [S]$ from $f$, then the CRPS may be estimated by
\begin{equation}
\text{CRPS}(f, \omega) \approx \frac{1}{S} \sum_{s = 1}^S | \phi_s - \omega | - \frac{1}{2S^2} \sum_{s = 1}^S \sum_{l = 1}^S | \phi_s - \phi_l |. (\#eq:crps-approx)
\end{equation}

A posterior marginal is calibrated if over repeated simulations
\begin{equation}
F(\omega) = q \sim \mathcal{U}[0, 1].
\end{equation}
In other words, if the quantile of the true value is uniform within the posterior marginal.
It follows that at any given nominal coverage $1 - \alpha$, the proportion of quantile-based credible intervals containing $\omega$ is also $1 - \alpha$.
Uniformity was assessed using:

* probability integral transform (PIT) histograms [@dawid1984present];
* empirical cumulative distribution function (ECDF) difference plots [@aldor2013power; @sailynoja2022graphical].
...
(ref:hiv-surveys) Figure caption.
...
The spatial random effect specification included in the model varied according to Table \@ref(tab:inferential-models).
...
Related work to discuss includes @follestad2003modelling, @konstantinoudis2020discrete.
