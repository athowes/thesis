...
Gram matrices were computed using the Matérn kernel $k: \mathcal{S} \times \mathcal{S} \to \mathbb{R}$ [@stein1999interpolation].
...
This value matches that used to simulate data, and simplifies Equation \@ref(eq:matern) to
\begin{equation}
...
Model performance was compared in estimating HIV prevalence $\rho_i \in [0, 1]$ in adults aged $15-49$ across four countries in sub-Saharan Africa: Côte d'Ivoire ($n = 33$), Malawi ($n = 28$), Tanzania ($n = 159$) and Zimbabwe ($n = 60$).
As before, the spatial random effect specification included in the model was varied.

### Household survey data

Data from the most recent publicly available Population Health Impact Assessment (PHIA) survey for each country was used.
