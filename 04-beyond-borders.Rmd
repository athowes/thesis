...
This study supports the conclusion that adjacency-based spatial random effect models are difficult to improve upon.
...
However, this finding only weakly supports use of kernel models for applied work, for the following reasons.
First, it is usually impossible to know that real data was generated from any particular process.
...
Previous comparisons of spatial random effects [@best2005comparison; @lee2011comparison] were limited to the DIC measure of model performance.
Use of the DIC is strongly discouraged by @vehtari2017practical.
This study used measures of model performance with less problems, such as the CRPS, calculated using cross-validation when required.
In Section \@ref(hiv-study) it would be beneficial to compute the DIC and WAIC in order to compare to findings based on cross-validation.
With increased sample size for the simulation study, choropleths of fit by area would enable more precise conclusions about spatial models.
...
In particular, the log-score requries a kernel density estimate of the posterior distribution.
Kernel density estimates can be very sensitive to hyperparameter estimates.
Meanwhile, the CRPS can be estimated without any tunable hyperparameters.
