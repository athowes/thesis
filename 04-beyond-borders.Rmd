...
Figures \@ref(fig:crps-mean-se-grid), \@ref(fig:crps-mean-se-civ) and \@ref(fig:crps-mean-se-tex) show mean CRPS values with 95% credible intervals which rarely overlap.

The average CRPS varied substantially between the three models used to simulate synthetic data, given Table \@ref(tab:simulation-models).
...
The results (Figure \@ref(fig:crps-mean-se-surveys)) for each survey were as follows:
...
Although these models are perhaps the most widely used, there are a range of other adjacency-based models, including the more general weighted ICAR model discussed in Section \@ref(weighted-icar).
...
None-the-less, it would be of value to confirm this empirically.
