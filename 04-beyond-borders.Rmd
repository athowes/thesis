...
```{r}
resource_version <- "20231122-105059-c6eec861"
```
...
This chapter investigates spatial random effect model specifications for areal data.
A simple model based on the adjacency structure between areas is popular in HIV small-area estimation and beyond.
The analysis aimed to determine if this model is adequate, or a more complex model is necessary.
...
Table: (\#tab:inferential-models) The models used for inference. Each model is implemented in the `arealutils` package. The BYM2 model was implemented using the sparsity preserving parameterisation described in Section 3.2 of @riebler2016intuitive.
...
Appendix \@ref(lengthscale-prior) analyses the sensitivity of the lengthscale posterior distribution to the choice of prior distribution.
...
Appendix \@ref(aghq-nuts) compares posterior mean and standard deviations to those obtained using the No-U-Turn Sampling (NUTS) Hamiltonian Monte Carlo (HMC) algorithm run using `Stan` [@carpenter2017stan] via the `tmbstan` package [@monnahan2018no].
...
(ref:crps-boxplot) Boxplots showing the minimum value, 25th quantile, median, 75th quantile and maximum value of the CRPS for each inferential model and simulation model. The mean value is shown as a point.

```{r crps-boxplot, fig.cap="(ref:crps-boxplot)"}
path <- paste0("resources/beyond-borders/", resource_version, "/depends/crps-boxplot.png")
knitr::include_graphics(path)
```

The BYM2 model performed similarly to the IID model on IID synthetic data, and similarly to the Besag model on Besag data.
The BYM2 proportion parameter was learnt (Appendix \@ref(bym2-proportion)).

The kernel models with a prior distribution on the lengthscale performed better than those which fixed the lengthscale based on a heuristic.
These models were able to successfully infer the lengthscale (Appendix \@ref(lengthscale)).
Discuss lengthscale to noise identifiability and posterior correlation.
...
However, it is difficult to capture the complexity of realistic applied scenarios.
Therefore, it is important to complement simulation studies with studies conducted on real data.
To this end, model performance was compared in estimating district-level HIV prevalence $\rho_i \in [0, 1]$ in adults aged 15-49 using household survey data across four countries in sub-Saharan Africa (Table \@ref(tab:hiv-surveys)).

| Country        | Survey    | Number of areas | Analysis level |
|----------------|---------- |-----------------|-----------------|
| Côte d'Ivoire  | PHIA 2017 | 33              | Region          |
| Malawi         | PHIA 2016 | 31              | Health district and cities, with the islands Likoma and Chizumulu removed |
| Tanzania       | PHIA 2017 | 31              | Region          |
| Zimbabwe       | PHIA 2016 | 60              | District        |

Table: (\#tab:hiv-surveys) Included household surveys.
...
In each country, data from the most recent publicly available Population Health Impact Assessment (PHIA) survey were used.
Sampling weights
\begin{equation}
w_{ij} = \frac{1}{\pi_{ij}}
\end{equation}
were used to account for the complex survey design whereby an individual $j$ in area $i$ has probability $\pi_{ij}$ of being included in the survey.
...
To provide comparison, the DIC and WAIC measures of model fit were also computed.
