...
The investigation was motivated by a genre of question often encountered during model construction.
Namely, should the model be expanded to capture a sensible, but hypothetical, feature of the data?
The hypothesised feature was that some areas are more related to each other than other areas, extending upon a base model which uses a binary relation.
As well as having practical origins, given that modelling of this relationship is particularly important in HIV mapping, the inquiry also stemmed from confusion in seeing widespread use of the base model in situations which would seem to violate its assumptions.
...
Modelling of spatial variation is especially important for the small-area estimation of HIV.
...
As a result, in previous small-area models of HIV including covariates only resulted in a modest improvement in predictions [Supplementary Figure 20, @dwyer2019mapping].
This lack of predictive covariates foregrounds the role of modelling spatial variation.
For mapping of other infectious diseases, such as Malaria where transmission is driven by more predictive and easily-measurable environmental factors, explanatory covariates are more easily available and modelling spatial variation is less important [@weiss2015re].
...
Perhaps the simplest GMRF model is that of @besag1991bayesian in which information is borrowed equally from each adjacent area.
...
In doing so, it provides practical recommendations for using models for areal spatial structure.
...
\end{equation}
The matrix $\mathbf{R}^{-}$ is the generalised inverse of the rank-deficient structure matrix $\mathbf{R}$, so-called because it defines the structure of the precision matrix, with entries
\begin{equation}
    R_{ij} =
    \begin{cases}
        n_{\delta i}, & i = j \\
...
\end{equation}
This density is a function of the pairwise differences $u_i - u_j$ and so is invariant to the addition of a constant $c$ to each entry $p(\mathbf{u}) = p(\mathbf{u} + c\mathbf{1})$, resulting in an improper uniform distribution on the average of the $u_i$.
...
@freni2018note recommended three best practices for use of the Besag model:
