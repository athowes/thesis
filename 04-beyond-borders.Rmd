...
Models were assessed using the continuous ranked probability score [CRPS; @matheson1976scoring].
...
Additionally, models were assessed according to their calibration.
A posterior marginal can be said to be calibrated if over repeated simulations, $F(\omega) = q \sim \mathcal{U}[0, 1]$.
...
Uniformity was assessed using probability integral transform (PIT) histograms [@dawid1984present] and empirical cumulative distribution function (ECDF) difference plots [@aldor2013power; @sailynoja2022graphical].

### Results

## HIV prevalence study {#hiv-study}

Model performance was compared in estimating HIV prevalence $\rho_i \in [0, 1]$ in adults aged $15-49$ across four countries in sub-Saharan Africa.
These were:
...
Sampling weights $w_{ij} = 1 / \pi_{ij}$ were used to account for complex survey design.

### Inferential models

Like in Section \@ref(simulation-study), the spatial random effect specification included in the model was varied according to Table \@ref(tab:inferential-models).
