...
For each of the three spatial random effect models and seven geometries 100 synthetic data were generated, resulting in a total of 2100 synthetic data.
...
| FIK   | $\mathbf{u} \sim \mathcal{N}(0, \tau_u^{-1} \mathbf{K})$ with $K_{ij} = K(A_i, A_j)$ as in Section \@ref(integrated-kernel) with fixed length-scale $l$ |
| IK    | $\mathbf{u} \sim \mathcal{N}(0, \tau_u^{-1} \mathbf{K})$ with $K_{ij} = K(A_i, A_j)$ as in Section \@ref(integrated-kernel) with length-scale prior distribution $l \sim \mathrm{Inv{\text-}Gamma}(a, b)$ with $a, b$ set based on the geometry |
...
Appendix \@ref(lengthscale-prior) illustrates the sensitivity of the lengthscale posterior distribution (Figure \@ref(fig:lengthscale-posterior)) to choice of six possible lengthscale prior distributions (Figure \@ref(fig:lengthscale-prior)).
...
Learning a lengthscale allowed the kernel models to perform well even for IID synthetic data.

For IID synthetic data, the spatial models performed worse on the interior of the geometry.
For spatial synthetic data, the spatial models perormed better on the interior of the geometry.
...
However, it is difficult to capture the complexity of a realistic applied scenario.
...
gt_cv_crps <- gt_cv_crps %>%
  gt::tab_caption(caption = 'The mean pointwise leave-one-out and spatial leave-one-out CRPS, with standard errors, for each inferential model across the four considered PHIA surveys. The units used in this table around thousandths.')
...
The results for each survey were as follows:

1. For the 2017 PHIA survey in Côte d'Ivoire, all of the models performed similarly.
Name of the city.
Investigate BYM2 hypothesis here.
Suggestion to include some type of covariates to overcome this challenge.

2. For the 2017 PHIA survey in Malawi...

3. For the 2017 PHIA survey in Tanzania...

4. Finally, for the 2016 PHIA survey in Zimbabwe

Figure \@ref(fig:crps-mean-se-surveys) and Table \@ref(tab:cv-crps).
Figures \@ref(fig:ladder-civ2017phia), \@ref(fig:ladder-mwi2016phia), \@ref(fig:ladder-tza2017phia), and \@ref(fig:ladder-zwe2016phia) show the posterior mean and 95% credible interval obtained for each area for each inferential model.
Figures \@ref(fig:crps-mean-se-civ2017phia), \@ref(fig:crps-mean-se-mwi2016phia), \@ref(fig:crps-mean-se-tza2017phia), and \@ref(fig:crps-mean-se-zwe2016phia).

## Discussion

### Modelling

* Kernel with long lengthscale looks like IID
...
* Previous comparisons of spatial random effects [@best2005comparison; @lee2011comparison] were limited to the DIC measure of model fit. Use of the DIC is recommended against. The metrics used in this study were more theoretically suitable and enabled more precise findings.
...
* How interpretable are the CRPS and MSE.
