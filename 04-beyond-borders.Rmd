...
Figures \@ref(fig:crps-mean-se-grid), \@ref(fig:crps-mean-se-civ) and \@ref(fig:crps-mean-se-tex) present the mean CRPS for the three realistic geometries.
Results for the four vignette geometries are provided in Appendix \@ref(simulation-appendix).
The most important findings from the simulation study are as follows:

* The difficulty of the inference problem varied across spatial random effect model used to generate synthetic data.
Namely, IID structure is harder to predict than either Besag or IK.
To a lesser extent, Besag structure is harder to predict than IK.
As would be expected, the greater amount of correlation structure, the easier the problem.

* For IID synthetic data, the IID, BYM2, CK and IK models perform well.
The BYM2, CK and IK models have ways to learn the extent of spatial structure.
For example, Appendix \@ref(bym2-proportion) shows successful recovery of the BYM2 proportion parameter.
Models with fixed spatial structure performed particularly poorly on the geometry interiors.

* For both Besag and IK synthetic data, the IID model performed poorly.
As with the IID case, the BYM2 proportion parameter enabled learning spatial structure, and as such the BYM2 model performed only modestly worse than the Besag model.

* Across all synthetic data, the CK and IK models with a prior distribution on the lengthscale performed better than the FCK and FIK models where the lengthscale was fixed.
These models were able to successfully infer the lengthscale (Appendix \@ref(lengthscale)).

* Although there were differences in the performance of the FCK and FIK models, the CK and IK models were hard to distinguish, even for geometries designed to be spatially irregular.
This is somewhat a puzzle.

* The lengthscale and the noise.
I wonder if kernel models might be defined where there hyperparameters are orthogonal.
...
1. For the 2017 PHIA survey in Côte d'Ivoire, all of the models performed similarly, using both LOO and SLOO cross-validation (Figures \@ref(fig:crps-mean-se-civ2017phia))
The pointwise CRPS value was especially high at the one outlying district in the survey, Grand-Ponts.
Though there are ways that this error possibly could be mitigated, such as using a distribution with wider tails than a Gaussian, it is difficult to see how any spatial model could have performed well in this case without additional covariates.

2. For the 2017 PHIA survey in Malawi (Figures \@ref(fig:crps-mean-se-mwi2016phia)).

3. For the 2017 PHIA survey in Tanzania (Figures  \@ref(fig:crps-mean-se-tza2017phia)).

4. Finally, for the 2016 PHIA survey in Zimbabwe (Figures \@ref(fig:crps-mean-se-zwe2016phia)).
...
* What are my recommendations with respect to which spatial model to use
