...
```{r}
resource_version <- "20231111-193806-8b573544"
```
...
The investigation was motivated by a question which often occurs during model construction: should the model be expanded to capture a reasonable, albeit hypothetical, feature of the data?
Here, the feature in question pertains to the spatial correlation structure between areas.
...
As a result, in previous small-area models of HIV have found including covariates to only result in a modest improvement in predictive performance [Supplementary Figure 20, @dwyer2019mapping].
...
* to model bird population dynamics from capture-recapture data [@saracco2010modeling];
...
(ref:geometry-graph) Panel A shows the districts of Zimbabwe. Panel B shows the corresponding adjacency graph $\mathcal{G}$ with vertices positioned in alignment with the area that they correspond to, and edges between adjacent areas.
...
This raises a number of concerns about the Besag model's applicability to this broader setting.
...
Though not in itself a problem, this fact does prompt consideration as to whether the class of geometries with the same adjacency graph is sufficiently similar to merit identical models.
...
In Panel C, the area with a longer shared border would be expected to be more highly correlated.
In Pancel D, the area with a closer centre of mass would be expected to be more highly correlated.
...
Indeed, more broadly it is reasonable to consider convolution random effects with any model for spatially structured noise.
...
\begin{equation}
    \phi \sim \mathcal{N} \left( 0, \frac{1}{\tau_\phi} \mathbf{K} \right), (\#eq:areal-kernel-sre)
\end{equation}
where the $n \times n$ Gram matrix $\mathbf{K}$ with entries $K_{ij} = K(A_i, A_j)$ is a valid covariance matrix.
The precision parameter $\tau_\phi$ is placed outside of the Gram matrix, analogous to the relation of the precision and structure matrices, but could be omitted.
...
Instead, most well-known spatial process models define the correlation structure between a points using a kernel function
\begin{equation}
k: \mathcal{S} \times \mathcal{S} \to \mathbb{R}. (\#eq:kernel-function)
\end{equation}
A simple method, and the one considered here henceforth, is to construct $K$ (Equation \@ref(eq:areal-kernel-function)) from $k$ (Equation \@ref(eq:kernel-function)) by averaging the kernel $k$ computed on some collection of points representing each area.
...
That said, it no surprising that a stationary covariance function would struggle to recover non-stationary structure.
...
In the machine learning literature, models of this kind have been studied under the name aggregated Gaussian processes [@law2018variational; @tanaka2019spatially; @yousefi2019multi; @hamelijnck2019multi].
Examples of use of this model in statistical practise are rare.
...
(ref:integration-strategy) The $n = 28$ districts of Malawi. Panel A shows the centroids as in Section \@ref(centroid-kernel). Panel B shows $L_i = 10$ randomly chosen points, Panel C hexagonal points, and Panel D grid points in each area, each generated using the `sf::st_sample` function [@pebesma2018sf].
...
Figure \@ref(fig:integration-strategy) shows...

These approaches require
\begin{equation}
\mathcal{O}(\sum_{i = 1}^n \sum_{j = 1}^n L_i L_j)
\end{equation}
evaluations of the kernel $k$ to compute the $n \times n$ Gram matrix $K$.
...
Rather than focusing on the aggregate nature of areal observations as a route towards better area-level estimates, disaggregation regression aims to produce high-resolution or point-level estimates from areal observations [@utazi2019spatial; @nandi2023disaggregation].
...
| IK    | $\mathbf{u} \sim \mathcal{N}(0, \mathbf{K}^\star)$ as in Section \@ref(integrated-kernel) with Matérn kernel, $\nu = 3/2, l = 2.5$ and $L_i = 100$ points per area |
...
(ref:geometries) Seven geometries were considered in the synthetic data study. These were the four geometries from Figure \@ref(fig:maup1) shown in Panel A, B, C and D, and three more realistic geometries shown in Panel E, F and G.
...
| Model | Details       |
|-------|---------------|
| Constant | Without spatial random effects |
...
| FCK   | $\mathbf{u} \sim \mathcal{N}(0, \tau_\phi^{-1} \mathbf{K})$ with $K_{ij} = k(c_i, c_j)$ as in Section \@ref(centroid-kernel) with fixed length-scale $l$ |
| CK    | $\mathbf{u} \sim \mathcal{N}(0, \tau_\phi^{-1} \mathbf{K})$ with $K_{ij} = k(c_i, c_j)$ as in Section \@ref(centroid-kernel) with length-scale prior distribution $l \sim \mathrm{Inv{\text-}Gamma}(a, b)$ |
| FIK   | $\mathbf{u} \sim \mathcal{N}(0, \tau_\phi^{-1} \mathbf{K})$ as in Section \@ref(integrated-kernel) with fixed length-scale $l$ |
| IK    | $\mathbf{u} \sim \mathcal{N}(0, \tau_\phi^{-1} \mathbf{K})$ as in Section \@ref(integrated-kernel) with length-scale prior distribution $l \sim \mathrm{Inv{\text-}Gamma}(a, b)$ |

Table: (\#tab:inferential-models) The models used for inference. Each model is implemented in the `arealutils` package.

Eight inferential models were fit to the synthetic data (Table \@ref(tab:inferential-models)).
Apart from the spatial random effect specification, each inferential model was the same as the simulation model.
...
The number of points per area $L_i$ was set to 10 with a hexagonal spacing structure (Panel C, Figure \@ref(fig:integration-strategy)).
...
A weakly informative half-Gaussian prior was placed on the standard deviation such that $\sigma_\phi \sim \mathcal{N}_+(0, 2.5^2)$ [@gelman2006prior].
The value 2.5 avoids placing significant prior density on the region $\sigma_\phi > 5$, which after logistic transformation would facilitate undesirable variation on the probability scale very close to zero or one.
...
In cases where a prior distribution was set on the length-scale it was $l \sim \mathrm{Inv{\text-}Gamma}(a, b)$, with $a$ and $b$ chosen for each geometry such that 1\% of the prior mass was below 0.1 and 1\% of the prior mass was above the maximum distance between points [@betancourt2017robust].
...
Models were implemented using a Template Model Builder C++ template for the log-posterior via the `TMB` R package [@kristensen2016tmb].
Appendix \@ref(aghq-nuts) compares approximate inference results to those obtained using the No-U-Turn Sampling (NUTS) Hamiltonian Monte Carlo (HMC) algorithm run using `Stan` [@carpenter2017stan] via the `tmbstan` package [@monnahan2018no].
...
Latent field parameter and hyperparameter marginals from each model were assessed using three methods:

* the mean squared error (MSE);
* the continuous ranked probability score [CRPS; @matheson1976scoring];
* the calibration.
...
The CRPS is a strictly proper scoring rule (SPSR) which "enjoys appealing properties and might serve as a standard score" [@gneiting2007strictly].
...
Uniformity was assessed using probability integral transform (PIT) histograms [@dawid1984present] and empirical cumulative distribution function (ECDF) difference plots [@aldor2013power] with simultaneous confidence bands [@sailynoja2022graphical].

### Results

## HIV prevalence study {#hiv-study}

Model performance was compared in estimating district-level HIV prevalence $\rho_i \in [0, 1]$ in adults aged 15-49 across four countries in sub-Saharan Africa.
These included countries were:
...
knitr::include_graphics(paste0("resources/beyond-borders/", resource_version, "/depends/hiv-surveys.png"))
```

Data from the most recent publicly available Population Health Impact Assessment (PHIA) survey were used for each country.
Sampling weights
\begin{equation}
w_{ij} = \frac{1}{\pi_{ij}}
\end{equation}
...
(ref:cv) In leave-one-out (LOO) cross-validation, in each fold one observation is left out of the training data and predicted upon. In spatial-leave-one-out (SLOO) cross-valdation, observations corresponding to adjacent areas are also left out of the training data.
...
1. a standard leave-one-out cross-validation (LOO-CV);
...
* Application of scoring rules to transformed versions of parameters [@bosse2023scoring].
* The log-score was considered but requires a kernel density estimate.
* Previous comparisons of spatial random effects [@best2005comparison; @lee2011comparison] were limited to the DIC measure of model fit. @spiegelhalter2014deviance discuss the flaws of the DIC. The metrics used in this study were more theoretically suitable and enabled more precise findings.
* Related work to discuss includes @follestad2003modelling and @konstantinoudis2020discrete.
* This study was facilitated by all models being implemented in `TMB`. Inference was then conducted using AGHQ over the marginal Laplace approximation using the `aghq` package. The accuracy of inferences was compared to gold-standard results from NUTS obtained using the `tmbstan` package. `TMB` is a fast and flexible tool for spatial modelling [@osgood2023statistical]. Chapter \@ref(naomi-aghq) extends `TMB` to implement the INLA algorithm of `R-INLA`.
* An earlier version of this study used `R-INLA`. Not all of the models were compatible with `R-INLA`, so `rstan` was used in some cases. However, this study design was flawed, as models cannot be directly compared when fit with differing inference methods. This problem was solved by using `TMB`. 
...
If the link function $g$ is the identity or linear then the two are equivalent.
