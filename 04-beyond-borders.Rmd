...
The investigation was motivated by a question which often occurs during model construction.
Namely, should the model be expanded to capture a reasonable, albeit hypothetical, feature of the data?
...
### Concerns about the Besag model
...
These models can be specified in terms of scaled weights $\{b_{ij}\}_{j \sim i}$ and a precision vector $\boldsymbol{\mathbf{\kappa}} = (\kappa_i)_{i \in [n]}$ such that
\begin{equation}
  u_i \, | \, \mathbf{u}_{-i} \sim \mathcal{N} \left( \sum_{j: j \sim i} b_{ij} u_j, \frac{1}{\kappa_i \tau_u} \right). (\#eq:weighted-icar-fc)
\end{equation}
For the Besag model, then $b_{ij} = 1 / n_{\delta i}$ and $\kappa_i = n_{\delta i}$.
...
In the machine learning literature, models of this kind have been studied in the  under the name aggregated Gaussian processes [@law2018variational; @tanaka2019spatially; @yousefi2019multi; @hamelijnck2019multi].
...
(ref:integration-strategy) Panel A centroids. Panel B randomly chosen points. Panel C hexagonal points. Panel D grid points.
...
For example, during MCMC when the kernel has hyperparamters which are learnt then the Gram matrix is recomputed for each proposed set of hyperparameters.
...
Models were first assessed using the continuous ranked probability score [CRPS; @matheson1976scoring], a strictly proper scoring rule [SPSR; @gneiting2007strictly].
...
\begin{equation}
\text{CRPS}(f, \omega) = \int_{-\infty}^{\infty} (F(\phi) - \mathbb{I} \{\phi \geq \omega \} )^2 \text{d}\phi.
\end{equation}
