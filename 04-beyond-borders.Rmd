...
Synthetic data were simulated from three known data generating processes.
Data $\y = (y_i)_{i \in [n]}$ were simulated from $y_i \sim \text{Bin}(m_i, \rho_i)$ where the probabilities $\rho_i \in [0, 1]$ were linked to linear predictors $\eta_i \in \mathbb{R}$ via
\begin{equation}
...
Seven geometries were considered.
These were the four vignette geometries from Figure \@ref(fig:maup1) which share an adjacency graph, and three more realistic geometries consisting of a $6 \times 6$ lattice grid, the 33 districts of Côte d'Ivoire and the 36 congressional districts of Texas.
The realistic geometries were chosen to represent plausible variation over spatial regularity for the small-area estimation setting.
For each spatial random effect and geometry 200 simulations were generated, resulting in 1400 total simulations.
